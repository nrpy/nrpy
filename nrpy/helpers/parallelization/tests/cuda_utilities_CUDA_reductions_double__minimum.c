...
__global__ static void find_global__minimum__cuda(REAL *data, REAL *min, uint const data_length) {
  ...
  if (warpID == 0) {
    ...
    if (tid == 0) {
      atomicMin((unsigned long long int *)min, *((unsigned long long int *)&local_reduced));
    }
  }
}
...
REAL find_global__minimum(REAL *data, uint const data_length) {
  ...
  *h_reduced = (ull)0xFFFFFFFFU;
  ...
  find_global__minimum__cuda<<<blockCount, threadCount>>>(data, d_reduced, data_length);
