...
__global__ static void find_global__sum__cuda(REAL *data, REAL *min, uint const data_length) {
...
REAL find_global__sum(REAL *data, uint const data_length) {
  ...
  *h_reduced = (ull)0U;
  ...
  find_global__sum__cuda<<<blockCount, threadCount>>>(data, d_reduced, data_length);
