...
#elif defined(__AVX__) // #ifdef __AVX512F__
#include <immintrin.h>
...
#define HorizAddSIMD(vec)                                                                                                                            \
  ({                                                                                                                                                 \
    const __m128d low_128 = _mm256_castpd256_pd128((vec));                                                                                           \
    const __m128d high_128 = _mm256_extractf128_pd((vec), 1);                                                                                        \
    const __m128d sum_128 = _mm_add_pd(low_128, high_128);                                                                                           \
    _mm_cvtsd_f64(sum_128) + _mm_cvtsd_f64(_mm_unpackhi_pd(sum_128, sum_128));                                                                       \
  })
...
#define HorizAddSIMD(vec_sum)                                                                                                                        \
  ({                                                                                                                                                 \
    const __m128d sum_128 = _mm_hadd_pd((vec_sum), (vec_sum));                                                                                        \
    _mm_cvtsd_f64(sum_128);                                                                                                                           \
  })
...
#define HorizAddSIMD(vec_sum)                                                                                                                        \
  ({                                                                                                                                                 \
    const __m128d temp_shuf = _mm_shuffle_pd((vec_sum), (vec_sum), 0x1);                                                                             \
    const __m128d sum_128 = _mm_add_pd((vec_sum), temp_shuf);                                                                                         \
    _mm_cvtsd_f64(sum_128);                                                                                                                           \
  })
#endif // __SSE3__
...
#endif // ifdef/elif block to determine which style, if any, SIMD is applicable

#endif // __SIMD_INTRINSICS_H__
