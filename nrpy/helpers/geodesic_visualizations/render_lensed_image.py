...
def _load_texture(image_input: Union[str, np.ndarray]) -> np.ndarray:
    """
    Utility to load and normalize textures into float64 arrays.
...
def generate_source_disk_array(
    ...
    """
    Generates a synthetic top-down texture of an accretion disk.
...
def generate_static_lensed_image(
    ...
    """
    The main rendering loop. It reads the binary blueprint, identifies where 
    each ray terminated, and maps that endpoint to a pixel on the final image.
    Uses 'pixel_accumulator' to handle potential super-sampling if multiple rays 
    hit the same pixel area.
    """
