...
"""
    ...
    for basename, expressions in (("hDD", hprimeDD), ("aDD", aprimeDD)):
        for i in range(3):
            for j in range(i, 3):
                access_gfs.append(
                    gri.CarpetXGridFunction.access_gf(gf_name=f"{basename}{i}{j}")
                )
...
""",
    ...
    schedule_poststep = (
        "ODESolvers_PostStep",
        f"""
schedule FUNC_NAME in ODESolvers_PostStep after {thorn_name}_evol_ApplyBCs before {thorn_name}_BSSN_constraints
{{
  LANG: C
  READS:  {reads}
  WRITES: {writes}
...
""",
    ...
    cfc.register_CFunction(
        ...
        ET_schedule_bins_entries=[schedule_initial, schedule_poststep],
    )
    return pcg.NRPyEnv()
...
if __name__ == "__main__":
    ...
    for validation_coord, validation_precompute in (
        ...
        poststep_schedule = dict(validation_schedules)["ODESolvers_PostStep"]
        ...
        if generated.count("CCTK_DEVICE(const Loop::PointDesc &p)") != 1:
            raise AssertionError("projection must contain one all-points device loop")
        write_section = generated.split("Step 2 of 2", maxsplit=1)[1]
        if write_section.count("GF(p.I) =") != 12:
            raise AssertionError("projection must store twelve outputs in one block")
        first_store = generated.index("GF(p.I) =", generated.index("Step 2 of 2"))
