...
"""
    ...
    for i in range(4):
        for j in range(i, 4):
            T4DD_access_gfs += [
                gri.CarpetXGridFunction.access_gf(
                    gf_name=f"T4UU{i}{j}")
            ]
            T4UU_expr_list += [T4UU[i][j]]

    loop_body = ""
    lapse_gf_access = gri.CarpetXGridFunction.access_gf(gf_name=f"alpha")
    loop_body += f"const CCTK_REAL alpha = {lapse_gf_access};\n"
    cf_gf_access = gri.CarpetXGridFunction.access_gf(gf_name=f"cf")
    ...
    for i in range(4):
        for j in range(i, 4):
            Tmunu_gf_access = gri.CarpetXGridFunction.access_gf(gf_name="eT"+coord_name[i]+coord_name[j], use_GF_suffix=False)
