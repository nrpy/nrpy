...
def regen__cuda__register_CFunction_read_checkpoint():
    """Regenerate trusted data stored in JSON file for register_CFunction_read_checkpoint"""
    from nrpy.infrastructures.gpu.checkpoints.cuda.checkpointing import register_CFunction_read_checkpoint
    ...
    with open("nrpy/infrastructures/gpu/checkpoints/tests/DOCTEST-cuda__register_CFunction_read_checkpoint.json",'w') as f:
        f.write(json.dumps(expected_str_dict, sort_keys=True, indent=4))

def regen__cuda__register_CFunction_write_checkpoint():
    """Regenerate trusted data stored in JSON file for register_CFunction_write_checkpoint"""
    from nrpy.infrastructures.gpu.checkpoints.cuda.checkpointing import register_CFunction_write_checkpoint
    from nrpy.helpers.generic import compress_string_to_base64
    ...
    with open("nrpy/infrastructures/gpu/checkpoints/tests/DOCTEST-cuda__register_CFunction_write_checkpoint.json",'w') as f:
        f.write(json.dumps(expected_str_dict, sort_keys=True, indent=4))
