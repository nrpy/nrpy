...
__global__ static void rfm_precompute_malloc__allocate_gpu(const size_t streamid, rfm_struct *restrict rfmstruct) {
  ...
  rfmstruct->f2_of_xx0 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS0);
  rfmstruct->f2_of_xx0__D0 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS0);
  rfmstruct->f2_of_xx0__DD00 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS0);
  rfmstruct->f4_of_xx1 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS1);
  rfmstruct->f4_of_xx1__D1 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS1);
  rfmstruct->f4_of_xx1__DD11 = (REAL *)malloc(sizeof(REAL) * d_params[streamid].Nxx_plus_2NGHOSTS1);
} // END FUNCTION rfm_precompute_malloc__allocate_gpu

/**
 * rfm_precompute_malloc: reference metric precomputed lookup arrays: malloc
 */
void rfm_precompute_malloc__rfm__SymTP(const commondata_struct *restrict commondata, const params_struct *restrict params,
...
} // END FUNCTION rfm_precompute_malloc__rfm__SymTP
