 ...
 */
__host__ __device__ void Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__Cylindrical(const params_struct *restrict params, const REAL xCart[3],
  ...
  Cartz -= params->Cart_originz;
  {
     ...
     *  "[xx[0] = sqrt(Cartx**2 + Carty**2)]"
     *  "[xx[1] = atan2(Carty, Cartx)]"
     *  "[xx[2] = Cartz]"
     */
    xx[0] = sqrt(((Cartx) * (Cartx)) + ((Carty) * (Carty)));
    xx[1] = atan2(Carty, Cartx);
    xx[2] = Cartz;
...
} // END FUNCTION: Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__Cylindrical
