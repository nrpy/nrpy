 ...
 */
__host__ __device__ void Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__SinhCylindrical(const params_struct *restrict params, const REAL xCart[3],
  ...
  Cartz -= params->Cart_originz;
  {
     ...
     *  "[xx[0] = params->SINHWRHO*asinh(sqrt(Cartx**2 + Carty**2)*sinh(1/params->SINHWRHO)/params->AMPLRHO)]"
     *  "[xx[1] = atan2(Carty, Cartx)]"
     *  "[xx[2] = params->SINHWZ*asinh(Cartz*sinh(1/params->SINHWZ)/params->AMPLZ)]"
     */
    xx[0] = params->SINHWRHO * asinh(sqrt(((Cartx) * (Cartx)) + ((Carty) * (Carty))) * sinh((1.0 / (params->SINHWRHO))) / params->AMPLRHO);
    xx[1] = atan2(Carty, Cartx);
...
} // END FUNCTION: Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__SinhCylindrical
