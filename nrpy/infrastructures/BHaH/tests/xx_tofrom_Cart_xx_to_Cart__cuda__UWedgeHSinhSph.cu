 ...
 */
__host__ __device__ void xx_to_Cart__rfm__UWedgeHSinhSph(const params_struct *restrict params, const REAL xx[3], REAL xCart[3]) {
  const REAL xx0 = xx[0];
  const REAL xx1 = xx[1];
  const REAL xx2 = xx[2];
  /*
...
} // END FUNCTION xx_to_Cart__rfm__UWedgeHSinhSph
