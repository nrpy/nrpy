 ...
 */
__host__ __device__ void Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__SinhCartesian(const params_struct *restrict params, const REAL xCart[3],
  ...
  Cartz -= params->Cart_originz;
  {
     ...
     *  "[xx[0] = params->SINHWXYZ*asinh(Cartx*sinh(1/params->SINHWXYZ)/params->AMPLXYZ)]"
     *  "[xx[1] = params->SINHWXYZ*asinh(Carty*sinh(1/params->SINHWXYZ)/params->AMPLXYZ)]"
     *  "[xx[2] = params->SINHWXYZ*asinh(Cartz*sinh(1/params->SINHWXYZ)/params->AMPLXYZ)]"
     */
    const REAL tmp0 = sinh((1.0 / (params->SINHWXYZ))) / params->AMPLXYZ;
    xx[0] = params->SINHWXYZ * asinh(Cartx * tmp0);
    xx[1] = params->SINHWXYZ * asinh(Carty * tmp0);
    xx[2] = params->SINHWXYZ * asinh(Cartz * tmp0);
...
} // END FUNCTION: Cart_to_xx_and_nearest_i0i1i2_assume_valid__rfm__SinhCartesian
