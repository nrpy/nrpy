...
@param[in] arr The array to search for the minimum in.
@param size The size of the array.
@param order The maximum array index shift to consider for the minimum.
@return The index of the local minimum in the array or -1 if no minimum is found.
"""
    cfunc_type = "size_t"
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    for (int shift = 1; shift <= order; ++shift) {
      ...
      size_t left_idx = NRPYMAX(i,shift) - shift; // returns 0 if i < shift else i - shift
      size_t right_idx = NRPYMIN(i+shift,size-1); // returns size-1 if i > size-1 else i + shift
