...
diagnostic_gfs; optionally exports the stored BSSN evolution state for raytracing
reconstruction; populates diagnostic_gfs via diagnostic_gfs_set(...); conditionally
calls diagnostics that consume diagnostic_gfs; releases temporary storage; and
...
def _register_CFunction_diagnostics(  # pylint: disable=unused-argument
    ...
    and on output steps it allocates diagnostic_gfs, optionally exports the stored BSSN
    evolution state for raytracing reconstruction, populates diagnostic_gfs via
 ...
 * the stored BSSN evolution state for raytracing reconstruction, initializes
...
def register_all_diagnostics(
    ...
    - Registers helper C functions for:
        ...
        - the optional raytracing-connections binary exporter.
    ...
    if enable_raytracing_connections_output:
        output_raytracing_connections_4d_data.register_CFunction_output_raytracing_connections_4d_data(
