...
def add_TP_utilities_to_Cfunction_dict(includes=None):
    ...
    prefunc = "// " + desc + "\n\n"
    prefunc += r"""
/* TwoPunctures:  File  "TP_utilities.c"*/
...
/* allocate an int vector with subscript range v[nl..nh] */
{
  ...
  if(retval == NULL)
    { fprintf(stderr,"allocation failure in ivector()"); exit(1); }
...
/* allocate a REAL vector with subscript range v[nl..nh] */
{
  ...
  if(retval == NULL)
    { fprintf(stderr,"allocation failure in dvector()"); exit(1); }
...
/* allocate a int matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  if(retval == NULL)
    { fprintf(stderr,"allocation failure (1) in imatrix()"); exit(1); }

  /* get all memory for the matrix in on chunk */
  retval[0] = malloc(sizeof(int)*(nrh-nrl+1)*(nch-ncl+1));
  if(retval[0] == NULL)
    { fprintf(stderr,"allocation failure (2) in imatrix()"); exit(1); }
...
/* allocate a double matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  if(retval == NULL)
    { fprintf(stderr,"allocation failure (1) in dmatrix()"); exit(1); }

  /* get all memory for the matrix in on chunk */
  retval[0] = malloc(sizeof(REAL)*(nrh-nrl+1)*(nch-ncl+1));
  if(retval[0] == NULL)
    { fprintf(stderr,"allocation failure (2) in dmatrix()"); exit(1); }
...
/* allocate a REAL 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  retval = malloc(sizeof(REAL **)*(nrh-nrl+1));
  if(retval == NULL)
    { fprintf(stderr,"allocation failure (1) in d3tensor()"); exit(1); }

  retval[0] = malloc(sizeof(REAL *)*(nrh-nrl+1)*(nch-ncl+1));
  if(retval[0] == NULL)
    { fprintf(stderr,"allocation failure (2) in d3tensor()"); exit(1); }

  /* get all memory for the tensor in on chunk */
  retval[0][0] = malloc(sizeof(REAL)*(nrh-nrl+1)*(nch-ncl+1)*(ndh-ndl+1));
  if(retval[0][0] == NULL)
    { fprintf(stderr,"allocation failure (3) in d3tensor()"); exit(1); }
...
/*--------------------------------------------------------------------------*/
void
free_d3tensor (REAL ***t, long nrl, long nrh, long ncl, long nch,
	       long ndl, long ndh)
...
/*--------------------------------------------------------------------------*/
void
chebft_Zeros (REAL u[], int n, int inv)
...
void
chebft_Extremes (REAL u[], int n, int inv)
...
/* --------------------------------------------------------------------------*/
REAL
chebev (REAL a, REAL b, REAL c[], int m, REAL x)
...
/* -----------------------------------------*/
void
fourder2 (REAL u[], REAL d2u[], int N)
{
  int l, l2, M, lpM;
...
/* -------------------------------------------------------------------------*/
REAL
scalarproduct (REAL *v, REAL *w, int n)
{
  int i;
  REAL result = 0;
