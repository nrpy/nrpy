...
def register_CFunction_TP_FuncAndJacobian() -> None:
    ...
    prefunc = f"// {desc}\n\n"
    prefunc += r"""/* TwoPunctures:  File  "FuncAndJacobian.c"*/
...
/* --------------------------------------------------------------------------*/
void
F_of_v (ID_persist_struct par,
        int nvar, int n1, int n2, int n3, derivs v, REAL *F,
        derivs u)
{
          ...
          for (k = 0; k < n3; k++)
            {
              ...
              NonLinEquations (par, sources[Index(0,i,j,k,1,n1,n2,n3)],
              ...
              if (debugfile && (k==0))
                {
                  ...
                  psi = 1.+
                    0.5 * par.par_m_plus  / r_plus +
                    0.5 * par.par_m_minus / r_minus +
                    U.d0[0];
                  psi2 = psi * psi;
                  // psi4 = psi2 * psi2;
                  // psi7 = psi * psi2 * psi4;
...
/* --------------------------------------------------------------------------*/
void
J_times_dv (ID_persist_struct par,int nvar, int n1, int n2, int n3, derivs dv,
	    REAL *Jdv, derivs u)
{				/*      Calculates the left hand sides of the non-linear equations F_m(v_n)=0*/
  /*      and the function u (u.d0[]) as well as its derivatives*/
  /*      (u.d1[], u.d2[], u.d3[], u.d11[], u.d12[], u.d13[], u.d22[], u.d23[], u.d33[])*/
...
/* --------------------------------------------------------------------------*/
void
JFD_times_dv (ID_persist_struct par, int i, int j, int k, int nvar, int n1, int n2,
	      int n3, derivs dv, derivs u, REAL *values)
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  ...
  REAL dV0, dV1, dV2, dV3, dV11, dV12, dV13, dV22, dV23, dV33,
...
/* --------------------------------------------------------------------------*/
void
SetMatrix_JFD (ID_persist_struct par, int nvar, int n1, int n2, int n3, derivs u,
	       int *ncols, int **cols, REAL **Matrix)
{
  int column, row, mcol;
                      ...
                      for (j1 = j_0; j1 <= j_1; j1++)
                        {
                          for (k1 = k_0; k1 <= k_1; k1++)
                            {
                              JFD_times_dv (par, i1, j1, k1, nvar, n1, n2, n3,
                                            dv, u, values);
...
REAL
PunctEvalAtArbitPosition (REAL *v, int ivar, REAL A, REAL B, REAL phi,
...
{
  REAL al = Pih * (2 * i + 1) / n1, be = Pih * (2 * j + 1) / n2,
...
/* --------------------------------------------------------------------------*/
REAL
interpol (REAL a, REAL b, REAL c, derivs v)
{
  return v.d0[0]
...
REAL
PunctTaylorExpandAtArbitPosition (ID_persist_struct par,int ivar, int nvar, int n1,
                                  int n2, int n3, derivs v, REAL x, REAL y,
                                  REAL z)
{
  REAL xs, ys, zs, rs2, phi, X, R, A, B, al, be, aux1, aux2, a, b, c,
    result, Ui;
  int i, j, k;
  derivs vv;
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
REAL
PunctIntPolAtArbitPosition (ID_persist_struct par,int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, REAL x, REAL y,
			    REAL z)
{
...
REAL
PunctEvalAtArbitPositionFast (REAL *v, int ivar, REAL A, REAL B, REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  REAL *p, *values1, **values2, result;
...
/* --------------------------------------------------------------------------*/
REAL
PunctIntPolAtArbitPositionFast (ID_persist_struct par,int ivar, int nvar, int n1,
                                int n2, int n3, derivs v, REAL x, REAL y,
                                REAL z)
{
  REAL xs, ys, zs, rs2, phi, X, R, A, B, aux1, aux2, result, Ui;
