...
static int
bicgstab (ID_persist_struct par,
          int const nvar, int const n1, int const n2, int const n3,
          derivs v, derivs dv,
          int const output, int const itmax, REAL const tol,
          REAL * restrict const normres);
static REAL
norm_inf (REAL const * restrict const F,
          int const ntotal);
static void
relax (REAL * restrict const dv,
       int const nvar, int const n1, int const n2, int const n3,
...
*/
static void
LineRelax_al (REAL * restrict const dv,
	      ...
	      REAL const * restrict const rhs,
              ...
              REAL **JFD);
static void
LineRelax_be (REAL * restrict const dv,
...
#pragma omp parallel for
      ...
      //ii =
      bicgstab (par,
                nvar, n1, n2, n3, v, dv, par.verbose, 100, dmax * 1.e-3, &normres);
#pragma omp parallel for
      for (int j = 0; j < ntotal; j++)
        v.d0[j] -= dv.d0[j];
      F_of_v (par, nvar, n1, n2, n3, v, F, u);
