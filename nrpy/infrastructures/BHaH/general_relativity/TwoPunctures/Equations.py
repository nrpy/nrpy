...
REAL
BY_KKofxyz (ID_persist_struct par, REAL x, REAL y, REAL z)
{
  int i, j;
  REAL r_plus, r2_plus, r3_plus, r_minus, r2_minus, r3_minus, np_Pp, nm_Pm,
  ...
  for (i = 0; i < 3; i++)
    {
      for (j = 0; j < 3; j++)
        {				/* Bowen-York-Curvature :*/
          Aij =
            + 1.5 * (par.par_P_plus[i] * n_plus[j] + par.par_P_plus[j] * n_plus[i]
                     + np_Pp * n_plus[i] * n_plus[j]) / r2_plus
            + 1.5 * (par.par_P_minus[i] * n_minus[j] + par.par_P_minus[j] * n_minus[i]
...
void
BY_Aijofxyz (ID_persist_struct par, REAL x, REAL y, REAL z, REAL Aij[3][3])
{
  int i, j;
  REAL r_plus, r2_plus, r3_plus, r_minus, r2_minus, r3_minus, np_Pp, nm_Pm,
...
/*-----------------------------------------------------------*/
void
NonLinEquations (ID_persist_struct par, REAL rho_adm,
		 ...
		 REAL y, REAL z, derivs U, REAL *values)
{
  ...
  psi =
    1. + 0.5 * par.par_m_plus / r_plus + 0.5 * par.par_m_minus / r_minus + U.d0[0];
  psi2 = psi * psi;
  psi4 = psi2 * psi2;
  psi7 = psi * psi2 * psi4;

  values[0] =
    U.d11[0] + U.d22[0] + U.d33[0] + 0.125 * BY_KKofxyz (par, x, y, z) / psi7 +
...
/*-----------------------------------------------------------*/
void
LinEquations (ID_persist_struct par, REAL A, REAL B, REAL X, REAL R,
	      REAL x, REAL r, REAL phi,
	      REAL y, REAL z, derivs dU, derivs U, REAL *values)
{
  ...
  values[0] = dU.d11[0] + dU.d22[0] + dU.d33[0]
    - 0.875 * BY_KKofxyz (par, x, y, z) / psi8 * dU.d0[0];
}
"""
