...
Includes SoA-aware synchronization, direct device allocation, and global conservation sync.
...
# Define the C-structures and macros required for the orchestrator
...
def batch_integrator_numerical(spacetime_name: str) -> None:
    ...
    desc = r"""@brief Main orchestrator with portable SoA architecture and full GPU offloading."""
    ...
    printf("Initializing Project Singularity-Axiom SoA architecture for %ld rays...\\n", num_rays);
    
    // Part 1: Dual-Architecture Master Memory Allocation
    ...
    #ifdef USE_GPU
        #pragma omp target teams distribute parallel for map(to: all_photons, commondata[0:1], tsm)
    #else
    ...
    for (long int i = 0; i < num_rays; i++) {{
        ...
        all_photons.affine_param_p[i] = 0.0;
        all_photons.affine_param_p_p[i] = 0.0;

        double w_d = n_z[0]*window_center[0] + n_z[1]*window_center[1] + n_z[2]*window_center[2];
        double w_val = all_photons.f[IDX_GLOBAL(1, i, num_rays)]*n_z[0] + all_photons.f[IDX_GLOBAL(2, i, num_rays)]*n_z[1] + all_photons.f[IDX_GLOBAL(3, i, num_rays)]*n_z[2] - w_d;
        ...
        double s_val = all_photons.f[IDX_GLOBAL(1, i, num_rays)]*commondata->source_plane_normal_x + all_photons.f[IDX_GLOBAL(2, i, num_rays)]*commondata->source_plane_normal_y + all_photons.f[IDX_GLOBAL(3, i, num_rays)]*commondata->source_plane_normal_z - s_d;
    ...
    // Initial Debug Log with SoA-Aware Sync
    if (commondata->debug_mode && fp_debug) {{
        #ifdef USE_GPU
            for(int k=0; k<9; k++) {{ #pragma omp target update from(all_photons.f[IDX_GLOBAL(k, 0, num_rays) : 1]) }}
        #endif
    ...
    // Part 2: Dual-Architecture Conservation Buffers
    double *initial_cq = NULL;
    if (commondata->perform_conservation_check) {{
        #ifdef USE_GPU
            initial_cq = (double *)omp_target_alloc(sizeof(double) * 5 * num_rays, omp_get_default_device());
        #else
            initial_cq = (double *)malloc(sizeof(double) * 5 * num_rays);
        #endif
        
        #ifdef USE_GPU
            #pragma omp target teams distribute parallel for map(to: all_photons, commondata[0:1]) is_device_ptr(initial_cq)
        #else
            #pragma omp parallel for
        #endif
        for (long int i = 0; i < num_rays; ++i) {{
        ...
        #ifdef USE_GPU
            // Pull results back to host for final comparison logic
            #pragma omp target update from(initial_cq[0 : 5 * num_rays])
        #endif
    }}
    ...
    // Part 3: Dual-Architecture Temporary Integration Buffers
    int *req_photon_ids; 
    double *req_pos, *metric_g4DD, *conn_GammaUDD, *k_array, *f_start, *f_temp, *f_out, *f_err;
    int *retry_keep_mask, *retry_scan_idx, *event_keep_mask, *event_scan_idx; 
    long int *compacted_active;
        ...
        conn_GammaUDD = (double *)omp_target_alloc(sizeof(double) * 40 * BUNDLE_CAPACITY, omp_get_default_device());
        k_array = (double *)omp_target_alloc(sizeof(double) * BUNDLE_CAPACITY * 6 * 9, omp_get_default_device());
        f_start = (double *)omp_target_alloc(sizeof(double) * BUNDLE_CAPACITY * 9, omp_get_default_device());
        f_temp  = (double *)omp_target_alloc(sizeof(double) * BUNDLE_CAPACITY * 9, omp_get_default_device());
        f_out   = (double *)omp_target_alloc(sizeof(double) * BUNDLE_CAPACITY * 9, omp_get_default_device());
        f_err   = (double *)omp_target_alloc(sizeof(double) * BUNDLE_CAPACITY * 9, omp_get_default_device());
        retry_keep_mask = (int *)omp_target_alloc(sizeof(int) * BUNDLE_CAPACITY, omp_get_default_device());
        retry_scan_idx = (int *)omp_target_alloc(sizeof(int) * BUNDLE_CAPACITY, omp_get_default_device());
        event_keep_mask = (int *)omp_target_alloc(sizeof(int) * BUNDLE_CAPACITY, omp_get_default_device());
        event_scan_idx = (int *)omp_target_alloc(sizeof(int) * BUNDLE_CAPACITY, omp_get_default_device());
        compacted_active = (long int *)omp_target_alloc(sizeof(long int) * BUNDLE_CAPACITY, omp_get_default_device());
    #else
        ...
        conn_GammaUDD = (double *)malloc(sizeof(double) * 40 * BUNDLE_CAPACITY);
        k_array = (double *)malloc(sizeof(double) * BUNDLE_CAPACITY * 6 * 9);
        f_start = (double *)malloc(sizeof(double) * BUNDLE_CAPACITY * 9);
        f_temp  = (double *)malloc(sizeof(double) * BUNDLE_CAPACITY * 9);
        f_out   = (double *)malloc(sizeof(double) * BUNDLE_CAPACITY * 9);
        f_err   = (double *)malloc(sizeof(double) * BUNDLE_CAPACITY * 9);
        retry_keep_mask = (int *)malloc(sizeof(int) * BUNDLE_CAPACITY);
        retry_scan_idx = (int *)malloc(sizeof(int) * BUNDLE_CAPACITY);
        event_keep_mask = (int *)malloc(sizeof(int) * BUNDLE_CAPACITY);
        event_scan_idx = (int *)malloc(sizeof(int) * BUNDLE_CAPACITY);
        compacted_active = (long int *)malloc(sizeof(long int) * BUNDLE_CAPACITY);
    #endif
    ...
    for (int i = initial_slot_idx; i >= 0 && active_photons > 0; i--) {{
        while (tsm.slot_counts[i] > 0) {{
            ...
            while (needs_retry_count > 0) {{
                #ifdef USE_GPU
                    #pragma omp target teams distribute parallel for map(to: all_photons) is_device_ptr(f_start)
                #else
                    #pragma omp parallel for
                #endif
                for(long int j=0; j<needs_retry_count; ++j) {{
                    for(int k=0; k<9; ++k) {{
                        f_start[IDX_LOCAL(k, j, BUNDLE_CAPACITY)] = all_photons.f[IDX_GLOBAL(k, needs_retry_indices[j], num_rays)];
                    }}
                }}
                ...
                for (int stage = 1; stage <= 6; ++stage) {{
                        ...
                        #pragma omp target teams distribute parallel for \
                                    map(to: all_photons) \
                                    is_device_ptr(f_start, k_array, f_temp, req_photon_ids, req_pos)
                    #else
                        #pragma omp parallel for
                    #endif
                    for (long int j = 0; j < needs_retry_count; ++j) {{
                        calculate_rkf45_stage_f_temp(stage, f_start, k_array, all_photons.h[needs_retry_indices[j]], f_temp, BUNDLE_CAPACITY, j);
                        req_photon_ids[j] = (int)needs_retry_indices[j];
                        for(int m=0; m<4; m++) req_pos[IDX_LOCAL(m, j, BUNDLE_CAPACITY)] = f_temp[IDX_LOCAL(m, j, BUNDLE_CAPACITY)];
                    }}
                    
                    {interpolation_func}(commondata, (int)needs_retry_count, req_photon_ids, req_pos, metric_g4DD, conn_GammaUDD);
                    
                    #ifdef USE_GPU
                        #pragma omp target teams distribute parallel for \
                                    map(to: all_photons) \
                                    is_device_ptr(f_temp, metric_g4DD, conn_GammaUDD, k_array)
                    #else
                        #pragma omp parallel for
                    #endif
                    for (long int j = 0; j < needs_retry_count; ++j) {{
                        calculate_ode_rhs(f_temp, metric_g4DD, conn_GammaUDD, k_array, BUNDLE_CAPACITY, stage, (int)j);
                    }}
                }}
                ...
                needs_retry_count = 0;
                    ...
                    #pragma omp target teams distribute parallel for \
                                ...
                                is_device_ptr(f_start, k_array, f_out, f_err)
                #else
                    #pragma omp parallel for
                #endif
                for (long int j = 0; j < current_retry_count; ++j) {{
                    long int p_idx = needs_retry_indices[j];
                    rkf45_kernel(f_start, k_array, all_photons.h[p_idx], f_out, f_err, BUNDLE_CAPACITY, j);
                    
                    double h_taken = all_photons.h[p_idx]; 
                    bool accepted = update_photon_state_and_stepsize(&all_photons, num_rays, p_idx, f_start, f_out, f_err, BUNDLE_CAPACITY, (int)j, commondata);

                    if (accepted) {{
                        for(int k=0; k<9; ++k) {{ 
                            all_photons.f_p_p[IDX_GLOBAL(k, p_idx, num_rays)] = all_photons.f_p[IDX_GLOBAL(k, p_idx, num_rays)]; 
                            all_photons.f_p[IDX_GLOBAL(k, p_idx, num_rays)] = f_start[IDX_LOCAL(k, j, BUNDLE_CAPACITY)]; 
                        }}
                        all_photons.affine_param_p_p[p_idx] = all_photons.affine_param_p[p_idx];
                        all_photons.affine_param_p[p_idx] = all_photons.affine_param[p_idx] - h_taken;
                        retry_keep_mask[j] = 0;

                        // SoA-Aware Sync for Debugging
                        if (commondata->debug_mode && p_idx == 0 && fp_debug) {{
                            #ifdef USE_GPU
                                for(int k=0; k<9; k++) {{ 
                                    #pragma omp target update from(all_photons.f[IDX_GLOBAL(k, 0, num_rays) : 1]) 
                                }}
                            #endif
                            #pragma omp critical
                            {{ 
                        ...
                        retry_keep_mask[j] = (all_photons.rejection_retries[p_idx] > commondata->rkf45_max_retries) ? 
                                             (all_photons.status[p_idx] = FAILURE_RKF45_REJECTION_LIMIT, 0) : 1;
                    }}
                }}
                
                if (current_retry_count > 0) {{
                    retry_scan_idx[0] = 0;
                    for (long int j = 1; j < current_retry_count; ++j) {{
                        retry_scan_idx[j] = retry_scan_idx[j-1] + retry_keep_mask[j-1];
                    }}
                    ...
                    for (long int j = 0; j < current_retry_count; ++j) {{
                        if (retry_keep_mask[j]) next_retry_indices[ retry_scan_idx[j] ] = needs_retry_indices[j];
                    }}
            ...
            #ifdef USE_GPU
                #pragma omp target teams distribute parallel for map(to: all_photons, commondata[0:1])
            #else
                #pragma omp parallel for
            #endif
            for (long int j = 0; j < bundle_size; ++j) {{
                ...
                event_keep_mask[j] = (all_photons.status[p_idx] == ACTIVE) ? 1 : 0;
            }}
            
            event_scan_idx[0] = 0;
            for (long int j = 1; j < bundle_size; ++j) {{
                event_scan_idx[j] = event_scan_idx[j-1] + event_keep_mask[j-1];
            }}
            ...
            for (long int j = 0; j < bundle_size; ++j) {{
                if (event_keep_mask[j]) compacted_active[ event_scan_idx[j] ] = bundle_photons[j];
            }}
    ...
    #endif
    for (long int i = 0; i < num_rays; i++) {{ 
        calculate_and_fill_blueprint_data_universal(commondata, &all_photons, num_rays, i, &results_buffer[i]); 
    }}

    // Part 4: Final Global Conservation Sync
    #ifdef USE_GPU
        #pragma omp target update from(all_photons.f[0 : 9 * num_rays])
    #endif
    ...
    if (commondata->perform_conservation_check && initial_cq != NULL) {{
        ...
        {{
            ...
            for (long int i = 0; i < num_rays; i++) {{
                ...
                {conserved_quantities_func}(commondata, all_photons.f, num_rays, i, &final_E, &final_Lx, &final_Ly, &final_Lz, &final_Q);

                double dE = (initial_cq[i*5] != 0) ? fabs((final_E - initial_cq[i*5]) / initial_cq[i*5]) : fabs(final_E - initial_cq[i*5]);
                ...
                if (commondata->a_spin == 0.0) {{
                    ...
                    dL = (initial_cq[i*5+3] != 0) ? fabs((final_Lz - initial_cq[i*5+3]) / initial_cq[i*5+3]) : fabs(final_Lz - initial_cq[i*5+3]);
                }}
                if (dL > local_max_dL) {{ local_max_dL = dL; local_worst_L = i; }}

                double dQ = (initial_cq[i*5+4] != 0) ? fabs((final_Q - initial_cq[i*5+4]) / initial_cq[i*5+4]) : fabs(final_Q - initial_cq[i*5+4]);
                if (dQ > local_max_dQ) {{ local_max_dQ = dQ; local_worst_Q = i; }}

                if (fp_cons_log) {{
                    #pragma omp critical
                    fprintf(fp_cons_log, "%ld %.6e %.6e %.6e %.6e %.6e %d %.6e\\n", 
                            i, dE, dL, dQ, initial_cq[i*5+4], final_Q, all_photons.status[i], all_photons.f[IDX_GLOBAL(0, i, num_rays)]);
                }}
            }}
            #pragma omp critical
            {{
    ...
    // Part 5: Complete Dual-Architecture Cleanup
    if (fp_debug) fclose(fp_debug);
        ...
        omp_target_free(conn_GammaUDD, omp_get_default_device()); 
        omp_target_free(k_array, omp_get_default_device()); 
        omp_target_free(f_start, omp_get_default_device()); 
        omp_target_free(f_temp, omp_get_default_device()); 
        omp_target_free(f_out, omp_get_default_device()); 
        omp_target_free(f_err, omp_get_default_device()); 
        omp_target_free(retry_keep_mask, omp_get_default_device()); 
        omp_target_free(retry_scan_idx, omp_get_default_device()); 
        omp_target_free(event_keep_mask, omp_get_default_device()); 
        omp_target_free(event_scan_idx, omp_get_default_device()); 
        omp_target_free(compacted_active, omp_get_default_device());
        if (initial_cq) omp_target_free(initial_cq, omp_get_default_device());
        ...
        free(retry_keep_mask); free(retry_scan_idx); free(event_keep_mask); 
        free(event_scan_idx); free(compacted_active);
        if (initial_cq) free(initial_cq);
        
        free(all_photons.f); free(all_photons.f_p); free(all_photons.f_p_p); 
        free(all_photons.affine_param); free(all_photons.affine_param_p); 
        free(all_photons.affine_param_p_p); free(all_photons.h); 
        free(all_photons.status); free(all_photons.rejection_retries); 
        free(all_photons.on_positive_side_of_window_prev); 
        free(all_photons.on_positive_side_of_source_prev); 
        free(all_photons.source_event_found); free(all_photons.source_event_lambda); 
        free(all_photons.source_event_f_intersect); free(all_photons.window_event_found); 
        free(all_photons.window_event_lambda); free(all_photons.window_event_f_intersect);
    #endif
