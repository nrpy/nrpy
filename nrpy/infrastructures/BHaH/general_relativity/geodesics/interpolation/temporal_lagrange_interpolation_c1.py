...
the reconstructed metric time derivative vanishes at ``t_numerical_initial``.
...
stencils that contain ``t_numerical_initial``; ordinary temporal interpolation remains
...
``temporal_lagrange_interpolation``. The code-generation
...
TEMPORAL_LAGRANGE_INTERP_C1_DEFINES = r"""
// Status codes returned by the C1 temporal interpolation helper.
typedef enum {
  TEMPORAL_LAGRANGE_INTERP_C1_SUCCESS = 0,
  TEMPORAL_LAGRANGE_INTERP_C1_INVALID_ORDER = 1,
  TEMPORAL_LAGRANGE_INTERP_C1_INVALID_INITIAL_TIME_NODE = 2
} temporal_lagrange_interp_c1_status; // END ENUM: temporal_lagrange_interp_c1_status
"""


def register_CFunction_temporal_lagrange_interpolation_c1(
    enable_simd: bool,
    project_dir: str,
) -> Union[None, pcg.NRPyEnv_type]:
    """
    Register the C1-constrained temporal interpolation helper.
    ...
    initial coordinate time ``t_numerical_initial``. The supplied strictly increasing
    ...
    has unit physical-time derivative at ``t_numerical_initial``. This preserves every
    ...
    Bdefines_h.register_BHaH_defines(
        ...
        TEMPORAL_LAGRANGE_INTERP_C1_DEFINES,
    )
...
initial numerical-spacetime time ``t_numerical_initial``. The stencil must contain
``t_numerical_initial`` exactly once. This helper preserves every supplied metric value
at its corresponding time node and enforces that the reconstructed metric has
zero coordinate-time derivative at ``t_numerical_initial``.

The helper first builds ordinary nonuniform barycentric Lagrange bases. It then
subtracts the metric's derivative at ``t_numerical_initial`` times a correction
polynomial that vanishes at every supplied node and has unit physical-time
derivative at ``t_numerical_initial``. The same correction is applied to each Cartesian
...
@param t_numerical_initial Initial numerical-spacetime coordinate time in the stencil.
@param t_target Target physical coordinate time at or above `t_numerical_initial`.
@param[out] g4dd_out Corrected interpolated metric components.
@param[out] g4dd_dD_out Corrected metric first derivatives in metric-pair
outermost storage order.
@return `TEMPORAL_LAGRANGE_INTERP_C1_SUCCESS` on success,
`TEMPORAL_LAGRANGE_INTERP_C1_INVALID_ORDER` for an unsupported order,
or `TEMPORAL_LAGRANGE_INTERP_C1_INVALID_INITIAL_TIME_NODE` when the
...
"""
  ...
  if (temporal_half_width < 0 ||
    ...
    return TEMPORAL_LAGRANGE_INTERP_C1_INVALID_ORDER;
  const int interp_order = 2 * temporal_half_width + 1;
  const REAL time_origin = t_numerical_initial;
  const REAL time_scale =
      (interp_order > 1) ?
          (slice_times[interp_order - 1] - slice_times[0]) :
          1.0;
  if (!(time_scale > 0.0) || t_target < t_numerical_initial)
    return TEMPORAL_LAGRANGE_INTERP_C1_INVALID_INITIAL_TIME_NODE;

  int initial_time_node = -1;
  for (int i = 0; i < interp_order; i++) {
    if (slice_times[i] == t_numerical_initial) {
  ...
  // Step 2: Normalize temporal coordinates about t_numerical_initial and build the
  ...
  // Step 4: Build the ordinary interpolation derivative basis at t_numerical_initial.
  ...
  // vanishes at t_numerical_initial while retaining every supplied metric value.
  ...
  return TEMPORAL_LAGRANGE_INTERP_C1_SUCCESS;
"""
