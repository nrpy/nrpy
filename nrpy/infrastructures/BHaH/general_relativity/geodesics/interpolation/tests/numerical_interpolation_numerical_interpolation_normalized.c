 ...
 * @param[in] d_spatial_stencil_center_i0 Trial-locked native dimension-0 centers, or NULL.
 * @param[in] d_spatial_stencil_center_i2 Trial-locked native dimension-2 centers, or NULL.
 * @param[in] d_integration_param_bundle Coordinate-time integration-parameter bundle.
...
void numerical_interpolation(const commondata_struct *restrict commondata, const params_struct *restrict params,
                             ...
                             termination_type_t *restrict d_status, const int *restrict d_spatial_stencil_center_i0,
                             const int *restrict d_spatial_stencil_center_i2, const double *restrict d_integration_param_bundle,
                             const double *restrict d_h, const int stage, double *restrict d_metric_bundle, double *restrict d_rhs_geometry_bundle,
                             const long int chunk_size, const int stream_idx) {
  (void)stream_idx;
...
#define GEOMETRY_SLICE(base_ptr, slice_idx, comp_idx) ((base_ptr)[(slice_idx) * TEMPORAL_LAGRANGE_INTERP_GEOMETRY_COMPONENT_COUNT + (comp_idx)])
  ...
  if (d_status == NULL) {
    fprintf(stderr, "ERROR: numerical_interpolation requires a non-NULL status bundle.\n");
    exit(1);
  ...
  if (numerical_window == NULL || numerical_window->slice_times == NULL || numerical_window->num_time_slices < 1ULL ||
    ...
    fprintf(stderr, "ERROR: numerical_interpolation received invalid mapped-window metadata.\n");
    exit(1);
  ...
  if (temporal_half_width < 0 || temporal_half_width > TEMPORAL_LAGRANGE_INTERP_MAX_HALF_WIDTH ||
    ...
    fprintf(stderr, "ERROR: numerical_interpolation received inconsistent temporal bounds or stencil metadata.\n");
    exit(1);
  ...
  if (temporal_num_points != numerical_window->temporal_interp_num_points) {
    fprintf(stderr, "ERROR: numerical_interpolation temporal stencil size disagrees with the mapped window.\n");
    exit(1);
  } // END IF: runtime stencil size mismatched

#pragma omp parallel for
  for (long int i = 0; i < chunk_size; i++) {
    if (d_status[i] == FAILURE_SPATIAL_INTERPOLATION || d_status[i] == FAILURE_TEMPORAL_INTERPOLATION) {
    ...
    const REAL rkf45_stage_time_fraction =
        (stage >= 1 && stage <= 6) ? (REAL[]){0.0, 1.0 / 4.0, 3.0 / 8.0, 12.0 / 13.0, 1.0, 1.0 / 2.0}[stage - 1] : NAN;
    ...
    if (d_spatial_stencil_center_i0 != NULL && d_spatial_stencil_center_i2 != NULL) {
    ...
    if (t <= t_numerical_initial) {
      ...
      if (window_status != TIME_WINDOW_MANAGER_NUMERICAL_SUCCESS || num_available_slices <= 0 ||
        ...
        if (spatial_status != AZIMUTHAL_SYMMETRY_SPATIAL_LAGRANGE_INTERP_SUCCESS) {
          ...
          for (int comp = 0; comp < num_available_slices * TEMPORAL_LAGRANGE_INTERP_G4_COMPONENT_COUNT; comp++)
            if (!isfinite((double)g4dd_available[comp]))
              interpolation_failure = FAILURE_SPATIAL_INTERPOLATION;
          if (d_rhs_geometry_bundle != NULL)
            for (int comp = 0; comp < num_available_slices * TEMPORAL_LAGRANGE_INTERP_GEOMETRY_COMPONENT_COUNT; comp++)
