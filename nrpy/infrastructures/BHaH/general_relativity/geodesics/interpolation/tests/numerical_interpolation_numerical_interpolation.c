...
#pragma omp parallel for
  ...
  // The user-facing t_numerical_end must identify the final stored numerical
  // slice to within two trusted dataset time steps.
  const REAL t_numerical_end_tolerance = 2.0 * dt_numerical_spacetime_data;
  ...
  if (temporal_half_width < 0 || temporal_half_width > TEMPORAL_LAGRANGE_INTERP_MAX_HALF_WIDTH ||
      temporal_half_width != numerical_window->temporal_interp_half_width || !isfinite((double)t_metric_0) || !isfinite((double)t_numerical_end) ||
      !isfinite((double)dt_numerical_spacetime_data) || dt_numerical_spacetime_data <= 0.0 || !isfinite((double)t_final_numerical_slice) ||
...
#pragma omp parallel for
  ...
  if (fabs((double)(t_numerical_end - t_final_numerical_slice)) > (double)t_numerical_end_tolerance) {
    fprintf(stderr,
            "ERROR: commondata->t_numerical_end=%e differs from the final numerical slice time=%e "
            "by more than 2*dt_numerical_spacetime_data=%e.\n",
            (double)t_numerical_end, (double)t_final_numerical_slice, (double)t_numerical_end_tolerance);
    exit(1);
  } // END IF: t_numerical_end was inconsistent with the final numerical slice time
    ...
    if (t < t_metric_0) {
      ...
      if (final_slice_payloads[0] == NULL) {
        ...
        const int spatial_status = azimuthal_symmetry_spatial_lagrange_interpolation__rfm__Spherical(
            spatial_context, commondata, params, x, y, z, 1, final_slice_payloads, g4dd_local, gamma4udd_local);
        if (spatial_status != AZIMUTHAL_SYMMETRY_SPATIAL_LAGRANGE_INTERP_SUCCESS)
          ray_failed = 1;
      } // END ELSE: final stored numerical slice was available for direct spatial interpolation
    } else {
            ...
            if (missing_is_upper == missing_is_lower) {
              ...
              if (missing_is_upper) {
                ...
                if (available_slice_indices[last_available_slot] != final_slice_index) {
                  ...
                  for (int comp = 0; comp < TEMPORAL_LAGRANGE_INTERP_GAMMA_COMPONENT_COUNT; comp++) {
                    gamma4udd_missing_local[comp] = GAMMA_SLICE(gamma4udd_available, last_available_slot, comp);
