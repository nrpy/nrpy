...
def conserved_quantities(spacetime_name: str, particle_type: str = "massive") -> None:
    ...
    # Step 2: Determine Active Quantities and Build Lists
    ...
    c_params = "const commondata_struct *restrict commondata, const double f[8]"
    ...
    # We unpack y[] into local 'const double' variables that match the SymPy symbols.

    preamble_lines = ["// Unpack position coordinates from f[0]..f[3]"]

    # 3.a: Unpack Coordinates (xx0, xx1, etc.)
    ...
    for i in range(4):
        # SymPy symbol is "p0", maps to y[4]
    ...
    desc = (
        f"@brief Computes conserved quantities for {spacetime_name} ({particle_type}).\n"
        f"Computed: {', '.join([v.strip('*') for v in list_of_c_vars])}.\n"
        "Expects state vector f[8] containing position and 4-momentum."
    )
