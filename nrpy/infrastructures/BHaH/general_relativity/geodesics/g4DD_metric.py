# nrpy/infrastructures/BHaH/general_relativity/geodesics/g4DD_metric.py
"""
Defines the C function for computing upper-triangular metric components.

This module provides the generator for an inline C function that calculates the ten
unique components of the covariant metric tensor for a specified spacetime. It parses
SymPy expressions representing the metric and extracts spatial coordinates based on
whether the particle is massive or a photon. Unpacking these coordinates from a
...
def g4DD_metric(
    ...
    """
    # Array size determination for preamble documentation
    if PARTICLE == "massive":
        array_size = 8
    ...
    for mu in range(4):
        for nu in range(mu, 4):
            ...
            # Map symbolic tensor components to the 1D thread-local C array
            list_of_g4DD_C_vars.append(f"metric_local[{k}]")
            k += 1

    # Extract coordinates dynamically to populate the preamble
    ...
    for i, symbol in enumerate(xx_symbols):
        ...
        if sym_name in used_symbol_names:
            # Unpack coordinates strictly into fast local registers
