...
"""
  ...
  const REAL dtheta = diagnosticstruct->dtheta; // dtheta is a REAL, so const is appropriate
"""
  ...
  for (int which_R_ext = 0; which_R_ext < num_of_R_exts; which_R_ext++) {
    ...
    if (num_dst_pts > 0) {
      ...
      for (int i = 0; i < num_dst_pts; i++) {
        dst_pts[i][0] = xx_shell_grid[which_R_ext][i][radial_like_index];
        dst_pts[i][1] = xx_shell_grid[which_R_ext][i][theta_like_index];
      }
