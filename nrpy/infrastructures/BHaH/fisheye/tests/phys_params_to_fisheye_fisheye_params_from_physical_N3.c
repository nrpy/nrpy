 ...
 */
int fisheye_params_from_physical_N3(commondata_struct *restrict commondata) {
  const int NTRANS = 3;
  const int NUNK = 2 * NTRANS;

  const REAL L = commondata->fisheye_phys_L;
  const REAL a[4] = {commondata->fisheye_a0, commondata->fisheye_a1, commondata->fisheye_a2, commondata->fisheye_a3};
  ...
  for (int i = 0; i < NTRANS; i++) {
    ...
    case 2:
      commondata->fisheye_R3 = Ri;
      commondata->fisheye_s3 = si;
      break;
    }
  }
