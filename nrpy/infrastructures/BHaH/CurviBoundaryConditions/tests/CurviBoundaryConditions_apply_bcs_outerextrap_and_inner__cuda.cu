  ...
  for (int which_gz = 0; which_gz < NGHOSTS; which_gz++) {
    for (int dirn = 0; dirn < 3; dirn++) {
      if (bc_info->num_pure_outer_boundary_points[which_gz][dirn] > 0) {
        ...
        const size_t threads_in_x_dir = BHAH_CURVIBC_EXTRAP_THREADS_IN_X_DIR;
        const size_t threads_in_y_dir = BHAH_CURVIBC_EXTRAP_THREADS_IN_Y_DIR;
        const size_t threads_in_z_dir = BHAH_CURVIBC_EXTRAP_THREADS_IN_Z_DIR;
