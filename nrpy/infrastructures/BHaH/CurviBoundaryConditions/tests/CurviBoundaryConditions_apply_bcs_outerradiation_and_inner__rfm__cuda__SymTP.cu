...
__device__ static inline void r_and_partial_xi_partial_r_derivs(const size_t streamid, const REAL xx0, const REAL xx1, const REAL xx2, REAL *r,
  ...
  const REAL tmp5 = ((bScale) * (bScale)) + tmp0;
  const REAL tmp6 = ((tmp3) * (tmp3)) * tmp5;
  const REAL tmp9 = sqrt(tmp5);
  const REAL tmp12 = tmp0 * tmp1 * tmp3 - tmp1 * tmp3 * tmp5;
  const REAL tmp16 = ((tmp1) * (tmp1)) * xx0 + ((tmp3) * (tmp3)) * xx0;
  const REAL tmp7 = tmp0 * ((tmp1) * (tmp1)) + tmp6;
  const REAL tmp8 = sqrt(tmp7);
  const REAL tmp13 = pow(tmp7, -3.0 / 2.0);
  const REAL tmp15 = (1.0 / sqrt(-tmp6 / tmp7 + 1));
  const REAL tmp10 = (1.0 / (tmp8));
  const REAL tmp14 = -tmp1 * tmp10 * tmp9 - tmp12 * tmp13 * tmp3 * tmp9;
  const REAL tmp17 = tmp10 * tmp3 * xx0 / tmp9 - tmp13 * tmp16 * tmp3 * tmp9;
  const REAL tmp18 = tmp15 / (tmp10 * tmp12 * tmp15 * tmp17 - tmp10 * tmp14 * tmp15 * tmp16);
  *r = tmp8;
  *partial_x0_partial_r = -tmp14 * tmp18;
  *partial_x1_partial_r = tmp17 * tmp18;
 ...
 *
 */
void apply_bcs_outerradiation_and_inner__rfm__SymTP(const commondata_struct *restrict commondata, const params_struct *restrict params,
...
} // END FUNCTION apply_bcs_outerradiation_and_inner__rfm__SymTP
