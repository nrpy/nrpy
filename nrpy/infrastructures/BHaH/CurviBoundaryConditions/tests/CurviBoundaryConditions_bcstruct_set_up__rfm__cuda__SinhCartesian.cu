...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cartesian (orig coord = SinhCartesian):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cartesian (orig coord = SinhCartesian)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate SinhCartesian):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = AMPLXYZ*(exp(xx0/SINHWXYZ) - exp(-xx0/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     *  "[yCart_from_xx = AMPLXYZ*(exp(xx1/SINHWXYZ) - exp(-xx1/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     *  "[zCart_from_xx = AMPLXYZ*(exp(xx2/SINHWXYZ) - exp(-xx2/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     */
    const REAL tmp0 = (1.0 / (SINHWXYZ));
    const REAL tmp2 = AMPLXYZ / (exp(tmp0) - exp(-tmp0));
    xCart_from_xx = tmp2 * (exp(tmp0 * xx0) - exp(-tmp0 * xx0));
    yCart_from_xx = tmp2 * (exp(tmp0 * xx1) - exp(-tmp0 * xx1));
    zCart_from_xx = tmp2 * (exp(tmp0 * xx2) - exp(-tmp0 * xx2));
  }
  ...
  {
    // xx_to_Cart_inbounds for Coordinate SinhCartesian):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = AMPLXYZ*(exp(xx0/SINHWXYZ) - exp(-xx0/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     *  "[yCart_from_xx_inbounds = AMPLXYZ*(exp(xx1/SINHWXYZ) - exp(-xx1/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     *  "[zCart_from_xx_inbounds = AMPLXYZ*(exp(xx2/SINHWXYZ) - exp(-xx2/SINHWXYZ))/(exp(1/SINHWXYZ) - exp(-1/SINHWXYZ))]"
     */
    const REAL tmp0 = (1.0 / (SINHWXYZ));
    const REAL tmp2 = AMPLXYZ / (exp(tmp0) - exp(-tmp0));
    xCart_from_xx_inbounds = tmp2 * (exp(tmp0 * xx0) - exp(-tmp0 * xx0));
    yCart_from_xx_inbounds = tmp2 * (exp(tmp0 * xx1) - exp(-tmp0 * xx1));
    zCart_from_xx_inbounds = tmp2 * (exp(tmp0 * xx2) - exp(-tmp0 * xx2));
  }
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in SinhCartesian coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( %.15e "
 ...
 */
void bcstruct_set_up__rfm__SinhCartesian(const commondata_struct *restrict commondata, const params_struct *restrict params, REAL *restrict xx[3],
...
} // END FUNCTION bcstruct_set_up__rfm__SinhCartesian
