...
void apply_bcs_inner_only(const commondata_struct *restrict commondata, const params_struct *restrict params, const bc_struct *restrict bcstruct,
  ...
  const size_t threads_in_x_dir = BHAH_CURVIBC_INNER_THREADS_IN_X_DIR;
  const size_t threads_in_y_dir = BHAH_CURVIBC_INNER_THREADS_IN_Y_DIR;
  const size_t threads_in_z_dir = BHAH_CURVIBC_INNER_THREADS_IN_Z_DIR;
