...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cylindrical (orig coord = SinhCylindricalv2n2):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cylindrical (orig coord = SinhCylindricalv2n2)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate SinhCylindricalv2n2):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = (rho_slope*xx0 + xx0**2*(AMPLRHO - rho_slope)*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))/(exp(1/SINHWRHO) -
     * exp(-1/SINHWRHO)))*cos(xx1)]"
     *  "[yCart_from_xx = (rho_slope*xx0 + xx0**2*(AMPLRHO - rho_slope)*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))/(exp(1/SINHWRHO) -
     * exp(-1/SINHWRHO)))*sin(xx1)]"
     *  "[zCart_from_xx = xx2**2*(AMPLZ - z_slope)*(exp(xx2/SINHWZ) - exp(-xx2/SINHWZ))/(exp(1/SINHWZ) - exp(-1/SINHWZ)) + xx2*z_slope]"
     */
    const REAL tmp0 = (1.0 / (SINHWRHO));
    const REAL tmp3 = (1.0 / (SINHWZ));
    const REAL tmp2 = rho_slope * xx0 + ((xx0) * (xx0)) * (AMPLRHO - rho_slope) * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
    xCart_from_xx = tmp2 * cos(xx1);
    yCart_from_xx = tmp2 * sin(xx1);
    zCart_from_xx = ((xx2) * (xx2)) * (AMPLZ - z_slope) * (exp(tmp3 * xx2) - exp(-tmp3 * xx2)) / (exp(tmp3) - exp(-tmp3)) + xx2 * z_slope;
  }
  ...
  {
    // xx_to_Cart_inbounds for Coordinate SinhCylindricalv2n2):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = (rho_slope*xx0 + xx0**2*(AMPLRHO - rho_slope)*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))/(exp(1/SINHWRHO) -
     * exp(-1/SINHWRHO)))*cos(xx1)]"
     *  "[yCart_from_xx_inbounds = (rho_slope*xx0 + xx0**2*(AMPLRHO - rho_slope)*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))/(exp(1/SINHWRHO) -
     * exp(-1/SINHWRHO)))*sin(xx1)]"
     *  "[zCart_from_xx_inbounds = xx2**2*(AMPLZ - z_slope)*(exp(xx2/SINHWZ) - exp(-xx2/SINHWZ))/(exp(1/SINHWZ) - exp(-1/SINHWZ)) + xx2*z_slope]"
     */
    const REAL tmp0 = (1.0 / (SINHWRHO));
    const REAL tmp3 = (1.0 / (SINHWZ));
    const REAL tmp2 = rho_slope * xx0 + ((xx0) * (xx0)) * (AMPLRHO - rho_slope) * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
    ...
    zCart_from_xx_inbounds = ((xx2) * (xx2)) * (AMPLZ - z_slope) * (exp(tmp3 * xx2) - exp(-tmp3 * xx2)) / (exp(tmp3) - exp(-tmp3)) + xx2 * z_slope;
  }
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in SinhCylindricalv2n2 coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( "
 ...
 */
void bcstruct_set_up__rfm__SinhCylindricalv2n2(const commondata_struct *restrict commondata, const params_struct *restrict params,
                                               REAL *restrict xx[3], bc_struct *restrict bcstruct, bc_struct *restrict bcstruct_device) {
#include "../set_CodeParameters.h"
