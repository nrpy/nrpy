...
__device__ static inline void r_and_partial_xi_partial_r_derivs(const size_t streamid, const REAL xx0, const REAL xx1, const REAL xx2, REAL *r,
  ...
  *r = r_slope * xx0 + tmp4 * tmp7;
  *partial_x0_partial_r = (1.0 / (r_slope + 2 * tmp4 * tmp5 * tmp6 * xx0 + tmp7 * (tmp0 * tmp2 + tmp0 * tmp3)));
 ...
 *
 */
void apply_bcs_outerradiation_and_inner__rfm__SinhSphericalv2n2(const commondata_struct *restrict commondata, const params_struct *restrict params,
...
} // END FUNCTION apply_bcs_outerradiation_and_inner__rfm__SinhSphericalv2n2
