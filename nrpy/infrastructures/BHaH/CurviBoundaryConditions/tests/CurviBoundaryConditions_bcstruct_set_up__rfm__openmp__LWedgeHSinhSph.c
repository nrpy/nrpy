...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cartesian (orig coord = LWedgeHSinhSph):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cartesian (orig coord = LWedgeHSinhSph)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate LWedgeHSinhSph):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  {
    // xx_to_Cart_inbounds for Coordinate LWedgeHSinhSph):
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in LWedgeHSinhSph coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( %.15e "
 ...
 */
void bcstruct_set_up__rfm__LWedgeHSinhSph(const commondata_struct *restrict commondata, const params_struct *restrict params, REAL *restrict xx[3],
...
} // END FUNCTION bcstruct_set_up__rfm__LWedgeHSinhSph
