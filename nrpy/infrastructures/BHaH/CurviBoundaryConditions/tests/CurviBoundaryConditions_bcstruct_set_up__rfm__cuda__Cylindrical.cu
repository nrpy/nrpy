...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cylindrical (orig coord = Cylindrical):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cylindrical (orig coord = Cylindrical)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate Cylindrical):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = xx0*cos(xx1)]"
     *  "[yCart_from_xx = xx0*sin(xx1)]"
     *  "[zCart_from_xx = xx2]"
     */
    xCart_from_xx = xx0 * cos(xx1);
    yCart_from_xx = xx0 * sin(xx1);
  ...
  {
    // xx_to_Cart_inbounds for Coordinate Cylindrical):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = xx0*cos(xx1)]"
     *  "[yCart_from_xx_inbounds = xx0*sin(xx1)]"
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in Cylindrical coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( %.15e "
 ...
 */
void bcstruct_set_up__rfm__Cylindrical(const commondata_struct *restrict commondata, const params_struct *restrict params, REAL *restrict xx[3],
...
} // END FUNCTION bcstruct_set_up__rfm__Cylindrical
