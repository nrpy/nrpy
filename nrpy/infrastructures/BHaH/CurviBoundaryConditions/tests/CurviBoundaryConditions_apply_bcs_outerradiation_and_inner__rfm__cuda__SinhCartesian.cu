 ...
 * Compute 1st derivative finite-difference derivative with arbitrary upwind
 */
__device__ static inline REAL FD1_arbitrary_upwind_x1_dirn(const size_t streamid, const REAL *restrict gf, const int i0, const int i1, const int i2,
                                                           const int offset) {
  MAYBE_UNUSED int const Nxx_plus_2NGHOSTS0 = d_params[streamid].Nxx_plus_2NGHOSTS0;
  MAYBE_UNUSED int const Nxx_plus_2NGHOSTS1 = d_params[streamid].Nxx_plus_2NGHOSTS1;
  MAYBE_UNUSED int const Nxx_plus_2NGHOSTS2 = d_params[streamid].Nxx_plus_2NGHOSTS2;
  REAL const invdxx1 = d_params[streamid].invdxx1;
  switch (offset) {
  case 0: {
    static const REAL Rational_decl__1_2 = 1.0 / 2.0;
    return (-Rational_decl__1_2 * gf[IDX3(i0, i1 - 1, i2)] + Rational_decl__1_2 * gf[IDX3(i0, i1 + 1, i2)]) * invdxx1;
  }
  case 1: {
    static const REAL Rational_decl__3_2 = 3.0 / 2.0;
    static const REAL Rational_decl__2_1 = 2;
    static const REAL Rational_decl__1_2 = 1.0 / 2.0;
    return (-Rational_decl__3_2 * gf[IDX3(i0, i1, i2)] + Rational_decl__2_1 * gf[IDX3(i0, i1 + 1, i2)] -
            Rational_decl__1_2 * gf[IDX3(i0, i1 + 2, i2)]) *
           invdxx1;
  }
  case -1: {
    static const REAL Rational_decl__1_2 = 1.0 / 2.0;
    static const REAL Rational_decl__2_1 = 2;
    static const REAL Rational_decl__3_2 = 3.0 / 2.0;
    return (+Rational_decl__1_2 * gf[IDX3(i0, i1 - 2, i2)] - Rational_decl__2_1 * gf[IDX3(i0, i1 - 1, i2)] +
            Rational_decl__3_2 * gf[IDX3(i0, i1, i2)]) *
           invdxx1;
  }
  }
...
__device__ static inline void r_and_partial_xi_partial_r_derivs(const size_t streamid, const REAL xx0, const REAL xx1, const REAL xx2, REAL *r,
  ...
  const REAL AMPLXYZ = d_params[streamid].AMPLXYZ;
  const REAL SINHWXYZ = d_params[streamid].SINHWXYZ;

  const REAL tmp0 = (1.0 / (SINHWXYZ));
  const REAL tmp6 = exp(tmp0) - exp(-tmp0);
  const REAL tmp2 = exp(tmp0 * xx2);
  const REAL tmp3 = exp(-tmp0 * xx2);
  const REAL tmp7 = ((AMPLXYZ) * (AMPLXYZ)) / ((tmp6) * (tmp6));
  const REAL tmp10 = exp(tmp0 * xx0);
  const REAL tmp11 = exp(-tmp0 * xx0);
  const REAL tmp15 = exp(tmp0 * xx1);
  const REAL tmp16 = exp(-tmp0 * xx1);
  const REAL tmp4 = tmp2 - tmp3;
  const REAL tmp12 = tmp10 - tmp11;
  const REAL tmp17 = tmp15 - tmp16;
  const REAL tmp24 = tmp0 * tmp15 + tmp0 * tmp16;
  const REAL tmp35 = tmp0 * tmp10 + tmp0 * tmp11;
  const REAL tmp8 = ((tmp4) * (tmp4)) * tmp7;
  const REAL tmp19 = ((tmp12) * (tmp12)) * tmp7 + ((tmp17) * (tmp17)) * tmp7;
  const REAL tmp29 = ((tmp4) * (tmp4)) * (2 * tmp0 * tmp2 + 2 * tmp0 * tmp3);
  const REAL tmp36 = ((tmp17) * (tmp17)) * (2 * tmp0 * tmp15 + 2 * tmp0 * tmp16);
  const REAL tmp39 = ((tmp12) * (tmp12)) * (2 * tmp0 * tmp10 + 2 * tmp0 * tmp11);
  const REAL tmp20 = tmp19 + tmp8;
  const REAL tmp21 = sqrt(tmp20);
  const REAL tmp28 = (1.0 / 2.0) / pow(tmp20, 3.0 / 2.0);
  const REAL tmp30 = 1 / (tmp19 * sqrt(1 - tmp8 / tmp20));
  const REAL tmp38 = (1.0 / 4.0) * pow(AMPLXYZ, 7) * tmp29 / (((tmp20) * (tmp20)) * pow(tmp6, 7));
  const REAL tmp27 = (1.0 / (tmp21));
  const REAL tmp37 = tmp30 * tmp35 * tmp36;
  const REAL tmp40 = tmp24 * tmp30 * tmp39;
  const REAL tmp45 = ((AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ)) * tmp28 * tmp4 / ((tmp6) * (tmp6) * (tmp6) * (tmp6) * (tmp6));
  const REAL tmp31 = tmp30 * (-((AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ)) * tmp28 * tmp29 / ((tmp6) * (tmp6) * (tmp6)) +
                              AMPLXYZ * tmp27 * (tmp0 * tmp2 + tmp0 * tmp3) / tmp6);
  const REAL tmp42 = (1.0 / 2.0) * ((AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ) * (AMPLXYZ)) * tmp27 / ((tmp6) * (tmp6) * (tmp6) * (tmp6));
  const REAL tmp43 = (1.0 / (tmp24 * tmp31 * tmp39 * tmp42 + tmp31 * tmp35 * tmp36 * tmp42 + tmp37 * tmp38 + tmp38 * tmp40));
  *r = tmp21;
  *partial_x0_partial_r = tmp12 * tmp24 * tmp31 * tmp43 * tmp7;
  *partial_x1_partial_r = tmp17 * tmp31 * tmp35 * tmp43 * tmp7;
  *partial_x2_partial_r = tmp43 * (tmp37 * tmp45 + tmp40 * tmp45);
...
__device__ static inline REAL compute_partial_r_f(const size_t streamid, REAL *restrict xx[3], const REAL *restrict gfs, const int which_gf,
  ...
  const REAL partial_x1_f = FD1_arbitrary_upwind_x1_dirn(streamid, &gfs[which_gf * ntot], dest_i0, dest_i1, dest_i2, i1_offset);
 ...
 *
 */
void apply_bcs_outerradiation_and_inner__rfm__SinhCartesian(const commondata_struct *restrict commondata, const params_struct *restrict params,
...
} // END FUNCTION apply_bcs_outerradiation_and_inner__rfm__SinhCartesian
