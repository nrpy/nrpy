...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cartesian (orig coord = UWedgeHSinhSph):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cartesian (orig coord = UWedgeHSinhSph)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate UWedgeHSinhSph):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = -AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*cos(xx1)/(exp(1/SINHW) - exp(-1/SINHW))]"
     *  "[yCart_from_xx = AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*sin(xx1)*sin(xx2)/(exp(1/SINHW) - exp(-1/SINHW))]"
     *  "[zCart_from_xx = AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*sin(xx1)*cos(xx2)/(exp(1/SINHW) - exp(-1/SINHW))]"
     */
  ...
  {
    // xx_to_Cart_inbounds for Coordinate UWedgeHSinhSph):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = -AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*cos(xx1)/(exp(1/SINHW) - exp(-1/SINHW))]"
     *  "[yCart_from_xx_inbounds = AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*sin(xx1)*sin(xx2)/(exp(1/SINHW) - exp(-1/SINHW))]"
     *  "[zCart_from_xx_inbounds = AMPL*(exp(xx0/SINHW) - exp(-xx0/SINHW))*sin(xx1)*cos(xx2)/(exp(1/SINHW) - exp(-1/SINHW))]"
     */
    const REAL tmp0 = (1.0 / (SINHW));
    const REAL tmp2 = AMPL * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
    const REAL tmp3 = tmp2 * sin(xx1);
    ...
    zCart_from_xx_inbounds = tmp3 * cos(xx2);
  }
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in UWedgeHSinhSph coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( %.15e "
 ...
 */
void bcstruct_set_up__rfm__UWedgeHSinhSph(const commondata_struct *restrict commondata, const params_struct *restrict params, REAL *restrict xx[3],
...
} // END FUNCTION bcstruct_set_up__rfm__UWedgeHSinhSph
