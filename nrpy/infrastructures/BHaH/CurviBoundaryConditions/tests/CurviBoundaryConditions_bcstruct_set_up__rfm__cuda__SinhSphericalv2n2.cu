...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Spherical (orig coord = SinhSphericalv2n2):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Spherical (orig coord = SinhSphericalv2n2)
  /*
   *  Original SymPy expressions:
  ...
  {
    // xx_to_Cart for Coordinate SinhSphericalv2n2):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) -
     * exp(-1/SINHW)))*sin(xx1)*cos(xx2)]"
     *  "[yCart_from_xx = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) -
     * exp(-1/SINHW)))*sin(xx1)*sin(xx2)]"
     *  "[zCart_from_xx = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) - exp(-1/SINHW)))*cos(xx1)]"
     */
    const REAL tmp0 = (1.0 / (SINHW));
    const REAL tmp2 = r_slope * xx0 + ((xx0) * (xx0)) * (AMPL - r_slope) * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
  ...
  {
    // xx_to_Cart_inbounds for Coordinate SinhSphericalv2n2):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) -
     * exp(-1/SINHW)))*sin(xx1)*cos(xx2)]"
     *  "[yCart_from_xx_inbounds = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) -
     * exp(-1/SINHW)))*sin(xx1)*sin(xx2)]"
     *  "[zCart_from_xx_inbounds = (r_slope*xx0 + xx0**2*(AMPL - r_slope)*(exp(xx0/SINHW) - exp(-xx0/SINHW))/(exp(1/SINHW) -
     * exp(-1/SINHW)))*cos(xx1)]"
     */
    const REAL tmp0 = (1.0 / (SINHW));
    const REAL tmp2 = r_slope * xx0 + ((xx0) * (xx0)) * (AMPL - r_slope) * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in SinhSphericalv2n2 coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( "
 ...
 */
void bcstruct_set_up__rfm__SinhSphericalv2n2(const commondata_struct *restrict commondata, const params_struct *restrict params, REAL *restrict xx[3],
...
} // END FUNCTION bcstruct_set_up__rfm__SinhSphericalv2n2
