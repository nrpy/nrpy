...
def regen_base_register_CFunction_MoL_step_forward_in_time():
    ...
    from nrpy.infrastructures.BHaH.MoLtimestepping.base_MoL import base_register_CFunction_MoL_step_forward_in_time, MoL_Functions_dict
    ...
    with open("nrpy/infrastructures/BHaH/MoLtimestepping/tests/DOCTEST-base_register_CFunction_MoL_step_forward_in_time.json",'w') as f:
        f.write(json.dumps(expected_str_dict, sort_keys=True, indent=4))
        
def regen__openmp__register_CFunction_MoL_step_forward_in_time():
    """Regenerate trusted data stored in JSON file for base_register_CFunction_MoL_step_forward_in_time"""
    from nrpy.infrastructures.BHaH.MoLtimestepping.base_MoL import MoL_Functions_dict
    from nrpy.infrastructures.BHaH.MoLtimestepping.openmp.MoL import register_CFunction_MoL_step_forward_in_time
    from nrpy.infrastructures.BHaH.MoLtimestepping.RK_Butcher_Table_Dictionary import (
        generate_Butcher_tables,
    )
    from nrpy.helpers.generic import compress_string_to_base64
    ...
    with open("nrpy/infrastructures/BHaH/MoLtimestepping/tests/DOCTEST-openmp__register_CFunction_MoL_step_forward_in_time.json",'w') as f:
        f.write(json.dumps(expected_str_dict, sort_keys=True, indent=4))

if __name__ == "__main__":
    regen_base_register_CFunction_MoL_step_forward_in_time()
    regen__openmp__register_CFunction_MoL_step_forward_in_time()