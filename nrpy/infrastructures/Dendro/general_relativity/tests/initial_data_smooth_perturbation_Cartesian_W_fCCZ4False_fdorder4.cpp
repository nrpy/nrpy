 ...
 */
void bssn_smooth_perturbation_block(const BlockGeometry &geom, DendroScalar *const *out_gfs, const DendroScalar smooth_perturbation_amplitude,
  ...
  for (int i2 = static_cast<int>(padding); i2 < static_cast<int>(nz - padding); i2++) {
    for (int i1 = static_cast<int>(padding); i1 < static_cast<int>(ny - padding); i1++) {
      for (int i0 = static_cast<int>(padding); i0 < static_cast<int>(nx - padding); i0++) {
        ...
        const DendroScalar tmp0 = 2 * M_PI / smooth_perturbation_wavelength;
        const DendroScalar smooth_profile = smooth_perturbation_amplitude * sin(tmp0 * xx0) * sin(tmp0 * xx2) * cos(tmp0 * xx1);
        out_aDD00[pp] += DendroScalar{1} * smooth_profile;
        out_aDD01[pp] += DendroScalar{2} * smooth_profile;
        out_aDD02[pp] += DendroScalar{3} * smooth_profile;
        out_aDD11[pp] += DendroScalar{4} * smooth_profile;
        out_aDD12[pp] += DendroScalar{5} * smooth_profile;
        out_aDD22[pp] += DendroScalar{6} * smooth_profile;
        out_alpha[pp] += DendroScalar{7} * smooth_profile;
        out_betU0[pp] += DendroScalar{8} * smooth_profile;
        out_betU1[pp] += DendroScalar{9} * smooth_profile;
        out_betU2[pp] += DendroScalar{10} * smooth_profile;
        out_cf[pp] += DendroScalar{11} * smooth_profile;
        out_hDD00[pp] += DendroScalar{12} * smooth_profile;
        out_hDD01[pp] += DendroScalar{13} * smooth_profile;
        out_hDD02[pp] += DendroScalar{14} * smooth_profile;
        out_hDD11[pp] += DendroScalar{15} * smooth_profile;
        out_hDD12[pp] += DendroScalar{16} * smooth_profile;
        out_hDD22[pp] += DendroScalar{17} * smooth_profile;
        out_lambdaU0[pp] += DendroScalar{18} * smooth_profile;
        out_lambdaU1[pp] += DendroScalar{19} * smooth_profile;
        out_lambdaU2[pp] += DendroScalar{20} * smooth_profile;
        out_trK[pp] += DendroScalar{21} * smooth_profile;
        out_vetU0[pp] += DendroScalar{22} * smooth_profile;
        out_vetU1[pp] += DendroScalar{23} * smooth_profile;
        out_vetU2[pp] += DendroScalar{24} * smooth_profile;
...
} // END FUNCTION: bssn_smooth_perturbation_block
