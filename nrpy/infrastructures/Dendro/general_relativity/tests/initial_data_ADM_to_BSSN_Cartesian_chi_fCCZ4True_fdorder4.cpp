 ...
 */
void fccz4_ADM_to_BSSN_block(const block_geometry_struct &geom, const DendroScalar *const *auxevol_gfs, DendroScalar *const *out_gfs) {
  ...
  const DendroScalar *const in_BU0 = auxevol_gfs[3] + geom.component_offset;
  const DendroScalar *const in_BU1 = auxevol_gfs[4] + geom.component_offset;
  const DendroScalar *const in_BU2 = auxevol_gfs[5] + geom.component_offset;
  const DendroScalar *const in_gammaDD00 = auxevol_gfs[6] + geom.component_offset;
  const DendroScalar *const in_gammaDD01 = auxevol_gfs[7] + geom.component_offset;
  const DendroScalar *const in_gammaDD02 = auxevol_gfs[8] + geom.component_offset;
  const DendroScalar *const in_gammaDD11 = auxevol_gfs[9] + geom.component_offset;
  const DendroScalar *const in_gammaDD12 = auxevol_gfs[10] + geom.component_offset;
  const DendroScalar *const in_gammaDD22 = auxevol_gfs[11] + geom.component_offset;
  const DendroScalar *const in_KDD00 = auxevol_gfs[12] + geom.component_offset;
  const DendroScalar *const in_KDD01 = auxevol_gfs[13] + geom.component_offset;
  const DendroScalar *const in_KDD02 = auxevol_gfs[14] + geom.component_offset;
  const DendroScalar *const in_KDD11 = auxevol_gfs[15] + geom.component_offset;
  const DendroScalar *const in_KDD12 = auxevol_gfs[16] + geom.component_offset;
  const DendroScalar *const in_KDD22 = auxevol_gfs[17] + geom.component_offset;
  ...
  for (int i2 = static_cast<int>(padding); i2 < static_cast<int>(nz - padding); i2++) {
    for (int i1 = static_cast<int>(padding); i1 < static_cast<int>(ny - padding); i1++) {
      for (int i0 = static_cast<int>(padding); i0 < static_cast<int>(nx - padding); i0++) {
        ...
        const DendroScalar FDPart3tmp6 =
            -FDPart3tmp1 - FDPart3tmp3 - FDPart3tmp5 + gammaDD00 * gammaDD11 * gammaDD22 + 2 * gammaDD01 * gammaDD02 * gammaDD12;
        ...
        const DendroScalar FDPart3tmp10 = FDPart3tmp7 * KDD00 * (gammaDD11 * gammaDD22 - ((gammaDD12) * (gammaDD12))) +
                                          FDPart3tmp7 * KDD11 * (gammaDD00 * gammaDD22 - ((gammaDD02) * (gammaDD02))) +
                                          FDPart3tmp7 * KDD22 * (gammaDD00 * gammaDD11 - ((gammaDD01) * (gammaDD01))) +
                                          FDPart3tmp9 * KDD01 * (-gammaDD01 * gammaDD22 + gammaDD02 * gammaDD12) +
                                          FDPart3tmp9 * KDD02 * (gammaDD01 * gammaDD12 - gammaDD02 * gammaDD11) +
                                          FDPart3tmp9 * KDD12 * (-gammaDD00 * gammaDD12 + gammaDD01 * gammaDD02);
        ...
        out_betU2[pp] = BU2;
        out_cf[pp] =
            (1.0 / cbrt(FDPart3tmp6 / (-FDPart3tmp1 * FDPart3tmp7 - FDPart3tmp3 * FDPart3tmp7 - FDPart3tmp5 * FDPart3tmp7 +
                                       FDPart3tmp7 * gammaDD00 * gammaDD11 * gammaDD22 + 2 * FDPart3tmp7 * gammaDD01 * gammaDD02 * gammaDD12)));
...
} // END FUNCTION: fccz4_ADM_to_BSSN_block
