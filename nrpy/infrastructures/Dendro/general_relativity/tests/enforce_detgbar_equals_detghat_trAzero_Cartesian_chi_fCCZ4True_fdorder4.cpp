 ...
 */
void fccz4_enforce_detgbar_equals_detghat_trAzero_block(const block_geometry_struct &geom, DendroScalar *const *in_gfs,
                                                        fccz4::generated::detgtrazero_status_struct *const status) {
  ...
  DendroScalar *const out_hDD00 = in_gfs[11] + geom.component_offset;
  DendroScalar *const out_hDD01 = in_gfs[12] + geom.component_offset;
  DendroScalar *const out_hDD02 = in_gfs[13] + geom.component_offset;
  DendroScalar *const out_hDD11 = in_gfs[14] + geom.component_offset;
  DendroScalar *const out_hDD12 = in_gfs[15] + geom.component_offset;
  DendroScalar *const out_hDD22 = in_gfs[16] + geom.component_offset;
  DendroScalar *const out_aDD00 = in_gfs[0] + geom.component_offset;
  DendroScalar *const out_aDD01 = in_gfs[1] + geom.component_offset;
  DendroScalar *const out_aDD02 = in_gfs[2] + geom.component_offset;
  DendroScalar *const out_aDD11 = in_gfs[3] + geom.component_offset;
  DendroScalar *const out_aDD12 = in_gfs[4] + geom.component_offset;
  DendroScalar *const out_aDD22 = in_gfs[5] + geom.component_offset;
      ...
      for (int i0 = static_cast<int>(padding); i0 < static_cast<int>(nx - padding); i0++) {
        ...
        if (!(det_ratio > 0) || !std::isfinite(det_ratio) || !std::isfinite(trace_residual)) {
          ...
          if (status->failed_points == 0) {
            ...
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD00[pp]))
              status->first_failing_field = 0;
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD01[pp]))
              status->first_failing_field = 1;
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD02[pp]))
              status->first_failing_field = 2;
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD11[pp]))
              status->first_failing_field = 3;
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD12[pp]))
              status->first_failing_field = 4;
            if (status->first_failing_field < 0 && !std::isfinite(in_aDD22[pp]))
              status->first_failing_field = 5;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD00[pp]))
              status->first_failing_field = 11;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD01[pp]))
              status->first_failing_field = 12;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD02[pp]))
              status->first_failing_field = 13;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD11[pp]))
              status->first_failing_field = 14;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD12[pp]))
              status->first_failing_field = 15;
            if (status->first_failing_field < 0 && !std::isfinite(in_hDD22[pp]))
              status->first_failing_field = 16;
        ...
        status->max_abs_det_minus_one = std::fmax(status->max_abs_det_minus_one, static_cast<double>(std::fabs(det_ratio - 1)));
        status->max_abs_trace_residual = std::fmax(status->max_abs_trace_residual, static_cast<double>(std::fabs(trace_residual)));
...
} // END FUNCTION: fccz4_enforce_detgbar_equals_detghat_trAzero_block
