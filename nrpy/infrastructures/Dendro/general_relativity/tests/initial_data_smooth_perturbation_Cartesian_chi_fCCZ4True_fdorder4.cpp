  ...
  for (int i2 = static_cast<int>(padding); i2 < static_cast<int>(nz - padding); i2++) {
    for (int i1 = static_cast<int>(padding); i1 < static_cast<int>(ny - padding); i1++) {
      for (int i0 = static_cast<int>(padding); i0 < static_cast<int>(nx - padding); i0++) {
        ...
        out_Theta_fCCZ4[pp] += DendroScalar{21} * smooth_profile;
        out_trK[pp] += DendroScalar{22} * smooth_profile;
        out_vetU0[pp] += DendroScalar{23} * smooth_profile;
        out_vetU1[pp] += DendroScalar{24} * smooth_profile;
        out_vetU2[pp] += DendroScalar{25} * smooth_profile;
...
} // END FUNCTION: fccz4_smooth_perturbation_block
