...
Metric, extrinsic-curvature, and unit-normal expressions are reused from
ExpansionFunctionTheta and evaluated on the surface with xx[0] -> hh.
...
- The tensors gammaDD[i][j], KDD[i][j], and sU[i] are given in this same
  spherical coordinate basis, at the surface points (reasonable, since they
  come from ExpansionFunctionTheta).
...
- Angular derivatives of surface gridfunctions are indexed as (theta, phi) -> (1, 2)
  since NRPy generates finite-difference derivative code in spherical coords.
...
- SE_qDD_dD[A][B][C]:
    Three-coordinate derivative array; angular derivatives use C in {1, 2} for NRPy codegen.
- SE_qDD_dDD[A][B][C][D]:
    Second derivative array for SE_qDD; angular derivatives use C, D in {1, 2}.
- SE_XD[B]:
    X_B = e_B^i K_ij s^j on the surface.
- SE_XD_dD[B][A]:
    Angular derivatives of X_B; angular derivatives use A in {1, 2}.
- SE_zeta:
    Scalar eigenfunction z; user-provided gridfunction.
- SE_zeta_dD[A], SE_zeta_dDD[A][B]:
    First and second derivatives of SE_zeta; angular derivatives use A, B in {1, 2}.
- SE_y_aux:
    Auxiliary scalar y for eigenproblem; user-provided gridfunction.
- SE_y_aux_dD[A], SE_y_aux_dDD[A][B]:
    First and second derivatives of SE_y_aux; angular derivatives use A, B in {1, 2}.
- SE_flux_densityU[A]:
    Surface flux density for div(R grad z), defined as
    sqrt(q) * R * q^{AB} * partial_B z.
- SE_flux_densityU_dD[A][C]:
    Angular derivatives of SE_flux_densityU with C in {1, 2}.

External BHaHAHA fields and generated derivative symbols used:

- hh:
    Surface radius h, provided by BHaHAHA as the evolved horizon gridfunction.
- hh_dD[A]:
    Coordinate derivatives of hh generated by NRPy finite-difference codegen;
    angular derivatives are hh_dD[1] and hh_dD[2].
- gammaDD[i][j]:
    3-metric gamma_ij in spherical basis (r, theta, phi).
    Imported from ExpansionFunctionTheta and evaluated on the surface.
- KDD[i][j]:
    Extrinsic curvature K_ij in the same basis as gammaDD.
    Imported from ExpansionFunctionTheta and evaluated on the surface.
- sU[i]:
...
class SpECTRESpinEstimateClass:
    ...
    def __init__(
        ...
        self._SE_qDD_dD = ixp.declarerank3("SE_qDD_dD", symmetry="sym01", dimension=3)
        ...
        for A in range(2):
            ...
            for B in range(2):
                tmp += (
                    self._sqrtq * self._R * self._SE_qUU[A][B] * self._SE_zeta_dD[B + 1]
                )
            self._SE_flux_densityU_expr[A] = tmp

        # Derivatives of flux; used for div(R grad z)
        self._SE_flux_densityU_dD = ixp.declarerank2("SE_flux_densityU_dD", dimension=3)
