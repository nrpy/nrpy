    ...
    for (
        ...
        if case_coord == "Cartesian":
            ...
            for i in range(3):
                for a in range(3):
                    for b in range(3):
                        crosswalk_coefficient = sp.diff(
                            expected_mewes_shift_delta[i]
                            - expected_alic_shift_delta[i],
                            validation_Bq.betaU_dD[a][b],
                        )
                        expected_crosswalk_coefficient = -rhs.Z4constraintU[
                            b
                        ] * sp.KroneckerDelta(i, a)
                        if (
                            sp.expand(
                                crosswalk_coefficient - expected_crosswalk_coefficient
                            )
                            != 0
                        ):
                            raise AssertionError(
                                "Mewes/Alic shift crosswalk coefficient changed"
                            )
                        ...
                        if (
                            sp.expand(
                                shift_gradient_coefficient
                                - expected_shift_gradient_coefficient
                            )
                            != 0
                        ):
                            raise AssertionError(
                                "Mewes shift-gradient coefficient changed"
                            )
                ...
                for a in range(3):
                    for b in range(3):
                        ...
                        if (
                            ...
                            raise AssertionError(
                                "aggregate Mewes shift-gradient coefficient changed"
                            )
