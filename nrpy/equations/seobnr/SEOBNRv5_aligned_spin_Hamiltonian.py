...
class SEOBNRv5_aligned_spin_Hamiltonian_quantities:
    ...
    def __init__(self) -> None:
        ...
        gap = (
            ...
            + (1 / self.r) * (
                sp.Rational(23, 32) * self.nu - sp.Rational(3, 32)
            )
        ...
        gam = (
            ...
            * (sp.Rational(15, 32) - sp.Rational(9, 32) * self.nu)
            + (1 / self.r) 
            * (sp.Rational(11, 32) * self.nu + sp.Rational(3, 32))
            + (self.pphi**4 / self.r**4) 
            * (
        ...
        self.xi = sp.sqrt(Dnons) * (Anons + ap**2 * u * u) / (
            1 + ap**2 * u * u)
        ...
        BnpalignSS = (
            (1 / self.r**3) * ( 
                ...
                + am**2 * (
                    self.nu * sp.Rational(3,4) 
                    - sp.Rational(3,16)
                )
            )
            + (1 / self.r**4) * ( 
                ap**2 * (
                    -self.nu * sp.Rational(1171,64) 
                    - sp.Rational(861,64)
                )
                + delta * ap * am * (
                    self.nu * sp.Rational(13,16) 
                    + sp.Rational(449,32)
                )
                + am**2 * (
        ...
        AalignSS = (
            (1 / self.r**4) * (
                ...
                + am**2 * (
                    self.nu * sp.Rational(1,2) 
                    + sp.Rational(1,8)
                )
            )
            + (1 / self.r**5) * (
                ap**2 * (
                    -self.nu * sp.Rational(175,64) 
                    - sp.Rational(225,64)
                )
                + delta * ap * am * (
                    -self.nu * sp.Rational(39,16) 
                    + sp.Rational(117,32)
                )
                + am**2 * (
    ...
    def diff1(self) -> None:
        """
        This function computes and sets the first derivatives 
        of the SEOBNRv5 aligned-spin Hamiltonian.
    ...
    def diff2(self) -> None:
        """
        This function computes and sets the second derivatives 
        of the SEOBNRv5 aligned-spin Hamiltonian.

        Inputs: 'dHreal_dr', 'r', 'pphi'
    ...
    def diff1_circ(self) -> None:
        ...
        This function computes and sets the first derivatives 
        of the SEOBNRv5 aligned-spin Hamiltonian 
        for the circular case (prstar = 0).
