...
step, recomputes same-slice Ricci/RHS data and evaluates the Cartesian
covariant four-metric plus its coordinate-time derivative from the native BSSN
evolution state using transformed symbolic recipes in
``nrpy.equations.general_relativity.geodesics.geodesics``, and writes a stable
binary payload for later raytracing.

The payload stores the final Cartesian metric and its coordinate-time
derivative directly.
...
def register_CFunction_output_raytracing_data(
    ...
    3. The 10 corresponding coordinate-time derivatives ``g4DDdt`` in the
       same upper-triangular order.

    The exporter evaluates Cartesian metric and time-derivative expressions
    only on interior points. Cartesian coordinates are evaluated on the full
    logical grid.
    ...
    # Step 1: Build the transformed symbolic recipes for the exported metric data.
    ...
    g4DD_dt = symbolic_g4DD_dt_recipe_from_bssn_grid_basis(
        bssn_coord_system=CoordSystem,
        target_basis="Cartesian",
        enable_bssn_rfm_precompute=enable_rfm_precompute,
    )
    ...
    metric_dt_components: List[Tuple[Tuple[int, int], str]] = [
        ((0, 0), "g4DDdt00"),
        ((0, 1), "g4DDdt01"),
        ((0, 2), "g4DDdt02"),
        ((0, 3), "g4DDdt03"),
    ...
    expr_list.extend(g4DD_dt[mu][nu] for (mu, nu), _ in metric_dt_components)
    lhs_list = [f"REAL {name}" for _, name in metric_components]
    lhs_list.extend(f"REAL {name}" for _, name in metric_dt_components)
    ...
    record_component_names.extend(name for _, name in metric_dt_components)
    header_label = "NRPy RT metric and dt data"
    record_component_name_bytes = 24
    header_label_bytes = 32
    format_name = "Cartesian g4DD+g4DDdt"
    ...
    point_record_real_count = 3 + num_metric_components + len(metric_dt_components)
...
"""
    ...
    for _, name in metric_components:
        ...
        component_index += 1
    for _, name in metric_dt_components:
        loop_body += (
            f"  payload_buffer[base_index + {component_index}] = (double){name};\n"
        )
        component_index += 1
    ...
    desc = """
Export final Cartesian-basis metric and metric time-derivative data for raytracing.
...
Each point record contains:
  ...
  - the 10 coordinate-time derivatives of those metric components in the same
      order.

Before evaluating the payload data, the exporter refreshes same-slice
...
function. The Cartesian tensor expressions are evaluated directly from the
transformed symbolic recipes in geodesics.py. The exporter does not perform a
second runtime tensor basis transformation after importing those recipes.

The exporter evaluates Cartesian coordinates on the full logical grid and
evaluates the Cartesian metric and time derivative only on interior logical-grid
points.
...
"""
  ...
  const char magic[16] = "NRPYRTMETRIC2";
...
"""
  ...
  // Step 4: Evaluate the Cartesian metric and its time derivative into interior tensor payload records.
"""
    body += loop
    body += r"""
