...
evaluates the first numerical slice directly or asks the numerical time-window
manager for one adaptive centered temporal stencil. In the mixed case, the
wrapper spatially interpolates only the mapped numerical stencil subset, fills
lower missing stencil nodes by freezing the first selected numerical slice,
fills upper missing stencil nodes by freezing the final selected numerical
slice, and then runs the temporal helper once on the reconstructed full stencil
to recover the final tensors at the photon coordinate time.
...
def register_CFunction_numerical_interpolation(
    ...
    shared mapped data. Rays at or below `t_metric_0` use the first selected
    numerical slice directly, treating the numerical spacetime as frozen in
    time before its first slice. Rays at or above the final selected numerical
    slice time use that final numerical slice directly, treating the numerical
    spacetime as frozen in time thereafter. Rays between those bounds use the
    adaptive
    ...
    numerical stencil subset, fill lower missing stencil nodes by reusing the
    first selected numerical slice, fill upper missing stencil nodes by reusing
    the final selected numerical slice, and then perform temporal interpolation
    on the reconstructed full stencil.
    ...
    :param analytical_metric_0: Name of the analytic metric retained for
        code-generation compatibility with the numerical integrator.
    ...
    _ = par.register_CodeParameter(
        ...
        add_to_parfile=True,
        description=(
            "Coordinate time of the first numerical slice. The numerical "
            "spacetime is frozen to this slice at earlier coordinate times."
        ),
    )
    _ = par.register_CodeParameter(
        "REAL",
    ...
    desc = r"""Interpolate piecewise analytic/numerical spacetime tensors for one photon chunk.
...
parallelizes over rays, freezes the numerical spacetime to the first selected
slice for times at or below `t_metric_0`, freezes it to the final selected slice
...
numerical slices with either one lower first-slice fill or one upper
final-slice fill before performing temporal interpolation at the photon
coordinate time.
...
"""
  ...
  for (long int i = 0; i < chunk_size; i++) {
    ...
    // Step 1: Dispatch directly to a frozen numerical boundary slice when
    // this ray is fully outside the mixed interpolation region.
    if (t <= t_metric_0) {
      const double *first_slice_payloads[1];
      first_slice_payloads[0] =
          time_window_manager_numerical_grid_ptr(numerical_window, 0ULL);
      ...
      if (window_status != TIME_WINDOW_MANAGER_NUMERICAL_SUCCESS ||
        ...
        if (spatial_status != AZIMUTHAL_SYMMETRY_SPATIAL_LAGRANGE_INTERP_SUCCESS)
          ...
          // lower missing nodes by freezing the first selected numerical
          // slice and fill upper missing nodes by freezing the final selected
          // numerical slice.
          if (num_missing_slices > 0) {
            ...
            if (missing_is_upper == missing_is_lower) {
              ...
              } else if (available_slice_indices[0] != 0ULL) {
                ray_failed = 1;
              } else {
                for (int comp = 0;
                     comp < TEMPORAL_LAGRANGE_INTERP_G4_COMPONENT_COUNT; comp++) {
                  g4dd_missing_local[comp] = G4_SLICE(g4dd_available, 0, comp);
                } // END LOOP: for comp over lower frozen metric components
                for (int comp = 0;
                     comp < TEMPORAL_LAGRANGE_INTERP_GAMMA_COMPONENT_COUNT; comp++) {
                  gamma4udd_missing_local[comp] =
                      GAMMA_SLICE(gamma4udd_available, 0, comp);
                } // END LOOP: for comp over lower frozen Christoffel components
              } // END ELSE: lower missing stencil edge reached the first selected numerical slice
