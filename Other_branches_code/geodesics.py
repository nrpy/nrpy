...
class GeodesicEquations:
    ...
    def _placeholder_adm_quantities_from_metric_and_connection() -> Tuple[
        ...
        r"""
        Reconstruct ADM quantities from generic metric and Christoffel placeholders.

        This helper reconstructs the lapse, shift, spatial metric, inverse spatial
        metric, extrinsic curvature, and the spatial derivatives needed by the
        Bohn-style normalized photon equations. It uses only the symbolic
        placeholder metric ``metric_g4DD`` and four-Christoffel symbols
        ``conn_Gamma4UDD`` so the resulting expressions can be consumed by the
        numerical photon build without requiring separate ADM payload fields.

        :return: Tuple containing ``alpha``, ``betaU``, ``gammaDD``, ``gammaUU``,
            ``KDD``, ``alpha_dD``, ``betaU_dD``, and ``gammaUU_dD``.
        """
        g4DD = ixp.declarerank2("metric_g4DD", symmetry="sym01", dimension=4)
        g4UU, _ = ixp.symm_matrix_inverter4x4(g4DD)
        Gamma4UDD = ixp.declarerank3("conn_Gamma4UDD", dimension=4, symmetry="sym12")
        ...
        for i in range(3):
            betaD[i] = g4DD[0][i + 1]
            betaU[i] = sp.sympify(0)
            for j in range(3):
                betaU[i] += gammaUU[i][j] * betaD[j]

        alpha = sp.sympify(1) / sp.sqrt(-g4UU[0][0])

        # Step 1: Reconstruct metric derivatives from metric compatibility:
        # nabla_lambda g_munu = 0
        # => partial_lambda g_munu
        #    = Gamma^alpha_mlambda g_alphanu + Gamma^alpha_nlambda g_mualpha
        g4DD_dD = ixp.zerorank3(dimension=4)
        for mu in range(4):
            for nu in range(mu, 4):
                for lam in range(4):
                    ...
                    for alpha_idx in range(4):
                        deriv += g4DD[mu][alpha_idx] * Gamma4UDD[alpha_idx][nu][lam]
                        deriv += g4DD[nu][alpha_idx] * Gamma4UDD[alpha_idx][mu][lam]
                    g4DD_dD[mu][nu][lam] = deriv
                    g4DD_dD[nu][mu][lam] = deriv

        # Step 2: Reconstruct the derivatives appearing in the normalized photon RHS.
        # The normalized Bohn equations need:
        # alpha_,i, beta^k_,i, gamma^jk_,i, and K_ij
        ...
        for i in range(3):
            # alpha = (-g^00)^(-1/2)
            # => alpha_,i = (1/2) alpha^3 (g^00)_,i
            # and (g^00)_,i = -g^0mu g^0nu g_munu,i
            g4UU00_dD = sp.sympify(0)
            for mu in range(4):
                for nu in range(4):
                    g4UU00_dD -= g4UU[0][mu] * g4UU[0][nu] * g4DD_dD[mu][nu][i + 1]
            alpha_dD[i] = sp.Rational(1, 2) * alpha**3 * g4UU00_dD

            # ADM identity already used on the photon side:
            # Gamma^0_ij = -K_ij / alpha
            # => K_ij = -alpha Gamma^0_ij
            for j in range(3):
                KDD[i][j] = -alpha * Gamma4UDD[0][i + 1][j + 1]

            # Inverse-metric derivative identity:
            # (gamma^jk)_,i = -gamma^ja gamma^kb gamma_ab,i
            for j in range(3):
                for k in range(3):
                    gammaUU_dD[j][k][i] = sp.sympify(0)
                    for a in range(3):
                        for b in range(3):
                            gammaUU_dD[j][k][i] -= (
                                gammaUU[j][a]
            ...
            # beta^k = gamma^kj beta_j
            # => beta^k_,i = (gamma^kj)_,i beta_j + gamma^kj beta_j,i
            for k in range(3):
                betaU_dD[k][i] = sp.sympify(0)
                for j in range(3):
                    betaU_dD[k][i] += (
        ...
        return alpha, betaU, gammaDD, gammaUU, KDD, alpha_dD, betaU_dD, gammaUU_dD
    ...
    def geodesic_eom_rhs_photon_normalized(self) -> List[sp.Expr]:
        ...
        As in the numerical photon pipeline, the required ADM quantities are
        reconstructed from the generic placeholder metric ``metric_g4DD`` and
        four-Christoffels ``conn_Gamma4UDD`` rather than from a separate ADM
        payload.
        ...
        """
        alpha, betaU, _, gammaUU, KDD, alpha_dD, betaU_dD, gammaUU_dD = (
        ...
        # Build the scalar contractions shared by du/dt and dPi_i/dt:
        # alpha_,j Pi^j
        # K_ij Pi^i Pi^j
        ...
        # Coordinate-time evolution:
        # dt/dt = 1
        # dx^i/dt = alpha Pi^i - beta^i
        pos_rhs = [sp.sympify(1)]
        for i in range(3):
            pos_rhs.append(alpha * PiU[i] - betaU[i])

        # du/dt = -alpha_,i Pi^i + alpha K_ij Pi^i Pi^j
        u_rhs = [-alpha_grad_dot_PiU + alpha * KDD_contract]

        # dPi_i/dt = -alpha_,i
        #          + (alpha_,j Pi^j - alpha K_jk Pi^j Pi^k) Pi_i
        #          + beta^k_,i Pi_k
        #          - (1/2) alpha gamma^jk_,i Pi_j Pi_k
        ...
        for i in range(3):
            ...
            inverse_metric_term = sp.sympify(0)
            for j in range(3):
                for k in range(3):
                    inverse_metric_term += gammaUU_dD[j][k][i] * PiD[j] * PiD[k]
            rhs -= sp.Rational(1, 2) * alpha * inverse_metric_term
            Pi_rhs[i] = rhs
        ...
        return pos_rhs + u_rhs + Pi_rhs + lambda_rhs
    ...
    def normalization_constraint_photon_normalized(self) -> sp.Expr:
        r"""
        Generate the normalized photon null constraint.
        ...
        This helper returns the left-hand side of that relation. A future
        normalized-photon diagnostic should therefore compare the returned
        scalar against ``1``.

        :return: A SymPy expression for ``\gamma^{ij} \Pi_i \Pi_j``.
        """
        _, _, _, gammaUU, _, _, _, _ = (
            self._placeholder_adm_quantities_from_metric_and_connection()
        )
        PiD = ixp.declarerank1("PiD", dimension=3)
        ...
        for i in range(3):
            for j in range(3):
                constraint += gammaUU[i][j] * PiD[i] * PiD[j]
