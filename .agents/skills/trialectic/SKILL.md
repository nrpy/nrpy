...
For ordinary work, produce three independent complete drafts, cross-critique them, synthesize the simplest sufficient candidate, validate it, and change live targets only after all three agents accept the same candidate and evidence. For NRPy KB work, use the review-only mode below.

This `SKILL.md` is the entire Trialectic implementation. Never require or add Trialectic-owned companion scripts, tests, metadata, templates, state files, or other skill files. Use existing project tools and task-relevant tests when warranted.

## Freeze scope

Before delegation, give all three agents one brief containing the exact request, acceptance criteria, target paths and create/revise/delete intent, initial target contents or absence, exact authorized implementation inputs, applicable ambient repository rules, and proportionate validation commands. Authorize individual paths: no globs, recursive discovery, neighboring-file reads, or writes outside targets. Preserve unrelated and concurrent work.

Never target, copy, inventory, or hash `AGENTS.md`. Treat harness-supplied repository instructions as ambient policy, not Trialectic inputs, and obey all protected-file rules.

### NRPy KB review mode

Trialectic may review KB candidates, but may not decide source authority, modify preserved sources, or install changes outside normal KB workflow.
...
Spawn the three fresh-context agents as independent reviewers of that same root-prepared candidate and evidence. They do not author competing KB drafts. Reviewers may identify defects and propose corrections, but source authority is determined by the KB schema and repository evidence, never agent consensus. Root alone applies accepted corrections through the normal KB workflow, reruns every applicable check, and freezes the revised candidate and evidence for a new review round. Root obtains all three reviewers' approval of the same exact candidate bytes and evidence before installation.
...
3. **NRPy coding-standards super expert:** Apply the shared NRPy standards with the deepest scrutiny, especially where architecture boundaries, generated output, validation policy, or compatibility are easy to miss.

For other work, use one first-principles analyst, one domain and implementation expert, and one adversarial editor and verifier. All three retain NRPy coding-style expertise and apply it whenever NRPy conventions govern the artifact.

## Draft or review, then cross-critique

Spawn exactly three fresh-context agents concurrently with separate work areas. Give them identical briefs plus their distinct roles and shared NRPy coding-style responsibilities. During independence, permit only exact authorized reads and each agent's own temporary writes; forbid peer reads and live writes. Require each complete draft to include all requested target operations and a short report with rationale, assumptions, inventory, role evidence, NRPy standards evidence, and final exact line `DRAFT COMPLETE`. Draft authors never approve their own draft.

In NRPy KB review mode, replace the draft-authoring instruction above with independent review of the same frozen root-prepared candidate and evidence. Each agent writes a complete review report; none creates an alternative candidate or edits candidate files. Each report must include rationale, assumptions, inventory, role evidence, NRPy standards evidence, and all defects or an explicit statement that none were found. Reviewers do not approve during this independent review phase.

After all three drafts or KB review reports complete, freeze them and verify completeness and scope before peer access. Do not run expensive full validation on every ordinary draft unless needed to make it reviewable. For ordinary work, have each agent critique both other agents' drafts. For KB work, have each agent critique both other agents' reviews against the same frozen candidate and evidence. Every material finding must name the exact location, violated requirement or standard, evidence, consequence, and smallest correction. Reviewers may expose defects but may not enlarge acceptance criteria or decide source authority.
...
For NRPy KB work, root resolves every material critique by correcting the single candidate only through the normal KB workflow. Root reruns every applicable workflow check and freezes the revised candidate and evidence. Any candidate or evidence change requires a new round with three fresh-context reviewers and three fresh decisions; reviewers never synthesize or install the candidate.
...
After preflight succeeds, freeze candidate bytes and all review and validation evidence. Any later change to candidate bytes or evidence creates a new candidate round and requires fresh validation and three fresh decisions.

## Require unanimity and install

Give all three agents the same frozen candidate, critique resolution, and validation evidence. Each independently returns either acceptance explicitly bound to those bytes and evidence or one concrete blocking defect. In KB review mode, acceptance confirms only that the candidate satisfies the brief and repository evidence; it cannot establish source authority. Silence, majority, stale review, or conditional approval does not pass.

Immediately before installation, confirm live targets and authorized inputs still match the shared brief. Drift restarts the workflow. After all three acceptances, apply only approved writes and deletions, verify installed state equals the accepted candidate, and rerun applicable installed-tree checks. For KB work, root performs these actions only through the normal KB workflow and leaves preserved sources unchanged. Report changed and deleted targets, validation results and limits, all three decisions, and temporary run location.
