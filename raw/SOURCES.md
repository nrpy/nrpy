...
| `dendro-trusted-generated-source-and-expression-baselines` | Trusted baselines from `nrpy/infrastructures/Dendro/general_relativity/tests`: 10 `.cpp` generated-source files for the small emitted kernels -- four initial-data builders and the algebraic-constraint enforcement, each at the conformal factor its application ships -- and 4 `.py` trusted expression dictionaries pinning the two right-hand sides and the two diagnostic sets, which are too large for a generated-source baseline. All captured and compared by the emitting modules' `__main__` sweeps. | living | partial |
...
| --- | --- | --- | --- | --- | --- |
| `https://github.com/paralab/Dendro-5.01` | Dendrolib, the host library Dendro-GR fetches as its `dendrolib` dependency, pinned to commit `246043709e806021fcfc011fe657b8bf964cae4c`; read from a local checkout that this repository does not track or commit. | frozen | 09-06-2026 | partial | Deciding source for the host-side block contract the generated Dendro solver assumes: `ot::Block` allocation sizes and pad width, and the unzip layout. Only the pieces the Dendro leaves cite are ingested. |
| `https://github.com/paralab/Dendro-GR` | The Dendro-GR application repository, which fetches Dendrolib as a dependency, read at commit `b3261e2a0d3457781b11d63ac5ab38375ffab93b` from a local checkout that this repository does not track or commit. | frozen | 09-07-2026 | partial | Source for the host-side conventions the generated Dendro solver is read beside: `BSSN_GR/src/bssnCtx.cpp`'s padded-origin arithmetic and block reads, `BSSN_GR/include/bssn_constraints.h`, `BSSN_GR/src/physcon.cpp`, `BSSN_GR/CMakeLists.txt`'s solver-name header comment, root `CMakeLists.txt`'s Dendrolib tag declaration before solver inclusion, and `BSSN_GR` namespace declarations. Only the pieces the KB cites are ingested. |
