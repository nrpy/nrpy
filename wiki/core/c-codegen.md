...
Some accepted options have narrower current behavior than their names suggest. `enable_clang_format` is consulted only at the end of `gridfunction_management_and_FD_codegen()`, so ordinary non-gridfunction C emission is not clang-formatted by this flag. On the `enable_fd_codegen` and `automatically_read_gf_data_from_memory` paths `c_codegen()` always passes the assembled finite-difference body through `order_statements_for_register_pressure()` (see [CSE And Printer Support](helpers/cse-and-printer-support.md)) before returning it, so whenever the body is scheduled the generated kernel no longer carries the read/derivative/expression phase comments; the helper returns the body unchanged, comments included, when it contains anything other than single-assignment `const` definitions and stores, or a load that follows a store. `enforce_c_parameters_must_be_defined` is stored but is not read elsewhere in `nrpy/c_codegen.py`; it currently enforces nothing. `SCALAR_TMP_varnames` and `SCALAR_TMP_sympyexprs` are caller-coordinated positional lists, and the implementation does not validate their paired lengths before indexing. Treat these as current implementation limits, not guarantees implied by option names.
...
- Dimensions: `platform=not-run; tool_version=not-run; backend=BHaH; precision=double; GPU=not-run; restart=not-applicable; distributed=not-applicable; error_path=not-run; options=list-valued upwind_control_vec, enable_fd_functions=False/True, enable_simd=False/True; date=09-07-2026`
