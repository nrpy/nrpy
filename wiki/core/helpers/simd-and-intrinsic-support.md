...
`simd_intrinsics.h` defines the C macros that generated SIMD code uses: `REAL_SIMD_ARRAY`, `SIMD_WIDTH`, load/store macros, constant construction, arithmetic, selected math functions, fused multiply-add/subtract spellings, absolute value, zero initialization, horizontal addition, and `UPWIND_ALG`. The header selects among AVX512F, AVX, SSE2/SSE3, and scalar fallback branches using compiler feature macros such as `__AVX512F__`, `__AVX__`, `__SSE2__`, `__SSE3__`, and `__FMA__`. Defining `NRPY_SIMD_AVX2` suppresses the AVX512F branch even when the compiler target provides it, choosing 256-bit AVX vectors under `-march=native`; callers without the override are unchanged. The scalar branch keeps the same macro names with width one, so generated code still compiles, one lane at a time, when the vector feature macros are absent.
