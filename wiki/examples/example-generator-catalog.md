...
| `dendro_bssn.py` | `python -m nrpy.examples.dendro_bssn [--project-dir DENDRO_GR_ROOT] [--fd-order {4,6,8}] [--conformal-factor {W,chi}] [--ko|--no-ko]` | Complete `NRPy_BSSN_GR` sibling application beside `BSSN_GR`, with CMake project `NRPy_BSSN_GR`, namespace `nrpy::bssn`, and executable `nrpyBssnSolver`; W and KO are defaults, while all FD4/6/8 kernels are emitted with the selected KO setting | Python for generation; Dendro-GR and Dendrolib checkout, MPI, C++17, and CMake for build | Manual/source-supported generation/build and TwoPunctures route; checked-in `dendro-validation` still targets the removed module layout, so it does not validate this application | [Dendro](../infrastructures/dendro/index.md) |
| `dendro_fccz4.py` | `python -m nrpy.examples.dendro_fccz4 [--project-dir DENDRO_GR_ROOT] [--fd-order {4,6,8}] [--conformal-factor {W,chi}] [--ko|--no-ko]` | Complete `NRPy_fCCZ4_GR` sibling application beside `BSSN_GR`, with CMake project `NRPy_fCCZ4_GR`, namespace `nrpy::fccz4`, and executable `nrpyFccz4Solver`; W and KO default on and all FD4/6/8 kernels are emitted with the selected KO setting | Python for generation; Dendro-GR and Dendrolib checkout, MPI, C++17, and CMake for build | Manual/source-supported generation/build and TwoPunctures route; checked-in `dendro-validation` still targets the removed module layout, so it does not validate this application | [Dendro](../infrastructures/dendro/index.md) |
...
Claim evidence:
- Claim: The Dendro generators emit complete sibling applications named `NRPy_BSSN_GR` and `NRPy_fCCZ4_GR`, each with KO enabled by default and a code-generation-time W/chi option; the checked-in Dendro CI job still targets the former module layout.
- Role: public generator interface and CI behavior
- Deciding authority: `nrpy/examples/dendro_bssn.py`, `parse_args`, `SOLVER_NAME`, and `main`; `nrpy/examples/dendro_fccz4.py`, `parse_args`, `SOLVER_NAME`, and `main`; `.github/workflows/main.yml`, `dendro-validation`.
...
- [dendro_bssn.py](../../nrpy/examples/dendro_bssn.py) - current BSSN command and sibling-application output.
- [dendro_fccz4.py](../../nrpy/examples/dendro_fccz4.py) - current fCCZ4 command and sibling-application output.
- [main.yml](../../.github/workflows/main.yml) - stale `dendro-validation` command and target layout.
