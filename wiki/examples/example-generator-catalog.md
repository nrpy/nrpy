...
| `photon_batch_geodesic_integrator_numerical.py` | `python -m nrpy.examples.photon_batch_geodesic_integrator_numerical --bin-name FILE --coord-system-numerical SinhCylindricalv2n2 --domain SIZE SINHWRHO SINHWZ RHO_SLOPE Z_SLOPE --t-numerical-end TF --dt-spacetime-data DT --t-start T --observer-position X Y Z --observer-look-forward X Y Z --observer-up UX UY UZ --escape-radius R --eom MODE --interpolation-method METHOD [--evolution-measure-max VALUE]` | CPU/OpenMP tiled photon raytracing project using a combined numerical-spacetime binary, common log-energy termination, and versioned light-blueprint records | Python, C compiler, `make`, and a combined BHaH raytracing dataset | Manual/source-supported numerical batch route | [Geodesic Raytracing](geodesic-raytracing.md) |
