...
| `dendro_bssn.py` | `python -m nrpy.examples.dendro_bssn [--project-dir DIR] [--fd-order {2,4,6}] [--ko/--no-ko]` | NRPy `nrpy_bssn` module under `project/<name>/Dendro-GR/`, with CMake project `nrpy_bssn` and namespace `nrpy::bssn`; child files, functions, and required Dendro targets keep the `bssn` stem (`bssnCtx.cpp`, `bssn_common`, `bssnSolver`) | Python for generation; MPI, a C++17 compiler, and a CMake generator to build the generated solver | Local standalone generation/build/CTest route; real-host BSSN execution remains open. | [Dendro](../infrastructures/dendro/index.md) |
| `dendro_fccz4.py` | `python -m nrpy.examples.dendro_fccz4 [--project-dir DIR] [--fd-order {2,4,6}] [--ko/--no-ko]` | NRPy `nrpy_fccz4` module under `project/<name>/Dendro-GR/`, with CMake project `nrpy_fccz4` and namespace `nrpy::fccz4`; child files, functions, and required Dendro targets keep the `fccz4` stem; KO is enabled by default | Python for generation; MPI, a C++17 compiler, and a CMake generator to build the generated solver | Configured `dendro-validation` nonflat numerical reference plus bounded two-rank, one-step Minkowski real-host check. | [Dendro](../infrastructures/dendro/index.md) |
...
| `massive_single_geodesic_integrator_analytical.py` | `python -m nrpy.examples.massive_single_geodesic_integrator_analytical` | Standalone massive-particle geodesic C project plus trajectory visualization copy | Python, C compiler, `make`, GSL, NumPy/Matplotlib for visualization | Manual/source-supported single-ray route | [Geodesic Raytracing](geodesic-raytracing.md) |
...
| `photon_batch_geodesic_integrator_analytical.py` | `python -m nrpy.examples.photon_batch_geodesic_integrator_analytical --observer-position X Y Z --observer-look-forward X Y Z --observer-up UX UY UZ --observer-fov W H --scan-density N --escape-radius R [--evolution-measure-max VALUE] [--cuda] [--outdir DIR]` | Standalone tiled photon raytracing project emitting versioned light-blueprint binary artifacts with an upper log-energy cutoff | Python, C or CUDA toolchain, `make`, NumPy/Matplotlib/Pillow for visualization; Numba optional for acceleration | Manual/source-supported batch route | [Geodesic Raytracing](geodesic-raytracing.md) |
| `photon_batch_geodesic_integrator_numerical.py` | `python -m nrpy.examples.photon_batch_geodesic_integrator_numerical --bin-name FILE --coord-system-numerical SinhCylindricalv2n2 --domain SIZE SINHWRHO SINHWZ RHO_SLOPE Z_SLOPE --t-numerical-end TF --dt-spacetime-data DT --t-start T --observer-position X Y Z --observer-look-forward X Y Z --observer-up UX UY UZ --observer-fov W H --scan-density N --escape-radius R --eom MODE --interpolation-method METHOD [--evolution-measure-max VALUE]` | CPU/OpenMP tiled photon raytracing project using a combined numerical-spacetime binary, common log-energy termination, and versioned light-blueprint records | Python, C compiler, `make`, and a combined BHaH raytracing dataset | Manual/source-supported numerical batch route | [Geodesic Raytracing](geodesic-raytracing.md) |
| `photon_single_geodesic_integrator_analytical.py` | `python -m nrpy.examples.photon_single_geodesic_integrator_analytical --spacetime NAME --eom MODE --observer-position X Y Z --observer-look-forward X Y Z --observer-up UX UY UZ --escape-radius R [--evolution-measure-max VALUE] [--disable-rkf45-trial-debug]` | Standalone single-photon geodesic project with a selectable symbolic spacetime recipe and EOM, unit observer energy, default-enabled RKF45 diagnostics, log-energy cutoff, and trajectory visualization copy | Python, C compiler, `make`, NumPy/Matplotlib for visualization | Manual/source-supported single-ray route | [Geodesic Raytracing](geodesic-raytracing.md) |
| `photon_single_geodesic_integrator_numerical.py` | `python -m nrpy.examples.photon_single_geodesic_integrator_numerical --bin-name FILE --coord-system-numerical SinhCylindricalv2n2 --domain SIZE SINHWRHO SINHWZ RHO_SLOPE Z_SLOPE --t-numerical-end TF --dt-spacetime-data DT --t-start T --observer-position X Y Z --observer-look-forward X Y Z --observer-up UX UY UZ --escape-radius R --eom MODE --interpolation-method METHOD [--evolution-measure-max VALUE] [--disable-rkf45-trial-debug]` | CPU/OpenMP single-photon geodesic project using a combined numerical-spacetime binary, selectable EOM, default-enabled RKF45 diagnostics, and common log-energy termination | Python, C compiler, `make`, and a combined BHaH raytracing dataset | Manual/source-supported numerical single-ray route | [Geodesic Raytracing](geodesic-raytracing.md) |
...
| `two_blackholes_collide.py` | `python -m nrpy.examples.two_blackholes_collide [--cuda] [--floating_point_precision TYPE] [--raytracing-time [T_FINAL DIAGNOSTICS_OUTPUT_EVERY]]` | Standalone BHaH Brill-Lindquist binary-black-hole project with optional raytracing-data export | Python, C or CUDA toolchain, `make`; BHaHAHA in supported OpenMP/double path | Configured Ubuntu/macOS default OpenMP build; local helper configures CUDA build; no runtime or raytracing-result check | [Standalone GR/BHaH](standalone-gr-bhah.md) |
...
| `nrpy/examples/geodesic_visualizations/*.py` | Trajectory plotter, light-blueprint schema and reader, lensed-image renderer, image visualizer, and blueprint diagnostics | `trajectory.txt`, PNGs, downloaded textures, and `light_blueprint_*.bin` files stay artifacts | [Geodesic Raytracing](geodesic-raytracing.md) |
...
- [two_blackholes_collide.py](../../nrpy/examples/two_blackholes_collide.py) - `parser`, `project_name`, `--raytracing-time`, `--raytracing-data-mode`
...
- [massive_single_geodesic_integrator_analytical.py](../../nrpy/examples/massive_single_geodesic_integrator_analytical.py) - `project_name`, `gsl-config`
- [photon_single_geodesic_integrator_analytical.py](../../nrpy/examples/photon_single_geodesic_integrator_analytical.py) - `project_name`
- [photon_batch_geodesic_integrator_analytical.py](../../nrpy/examples/photon_batch_geodesic_integrator_analytical.py) - `--outdir`, `--cuda`, `project_name`
- [photon_single_geodesic_integrator_numerical.py](../../nrpy/examples/photon_single_geodesic_integrator_numerical.py) - numerical-spacetime CLI, `project_name`
- [photon_batch_geodesic_integrator_numerical.py](../../nrpy/examples/photon_batch_geodesic_integrator_numerical.py) - numerical-spacetime CLI, `project_name`
- [normal_observer_log_energy.py](../../nrpy/infrastructures/BHaH/general_relativity/geodesics/photon/normal_observer_log_energy.py) - direct-EOM normal-observer log-energy helper
