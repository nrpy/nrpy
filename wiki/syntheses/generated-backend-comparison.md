...
fCCZ4 can also be added to a Dendro-GR checkout.
...
| Dendro | Dendro infrastructure over the NRPy gridfunction, CodeParameter, and `CFunction` registries, read directly by the emitter for each generated file; fCCZ4 and BSSN are the applications. | Generated solver directory under `project/<name>/Dendro-GR/<solver_name>/`: state and parameter headers, one source per registered CFunction, solver and tests CMake, a sample parameter file, generated self-tests, the host context and entry point, and the standalone host header. | Run the Python generator, configure the solver with CMake, build it, then run the generated executable; it compiles against a standalone host header and needs MPI and a C++17 compiler. fCCZ4 additionally supports `FCCZ4_STANDALONE_HOST=OFF`, which selects real Dendro block/vector/context types and host targets; BSSN is standalone-only. | `dendro-validation` runs the fCCZ4 nonflat numerical reference and a bounded two-rank, one-step Minkowski real-host check. Configuration defines reproducible required checks; it does not store an execution result in the KB. | Cite the Dendro emitter modules, registry symbols, and capability oracles instead of generated solver files and binaries. |
...
third-party host it does not vendor. Every generated solver supports the
standalone test executable; fCCZ4 also supports the tested real-host build. The
configured fCCZ4 CI route checks fixed-mesh
Minkowski behavior; general boundaries, remeshing, distributed transport,
and BSSN real-host execution require separate tests.
...
Claim evidence:
- Claim: every generated Dendro solver has a standalone build, while only fCCZ4 has a qualified real-host branch. The configured real fCCZ4 CI route checks fixed-mesh Minkowski; this does not qualify general boundaries, remeshing, distributed transport, or BSSN real-host execution.
- Role: descriptive behavior
...
- Corroboration: [main.yml](../../.github/workflows/main.yml), configured `dendro-validation` route
