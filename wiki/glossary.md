...
> Canonical spellings for recurring code, domain, and experiment entities. · Status: confirmed · Last reconciled: 09-06-2026
...
| Dendro | Generated solver infrastructure target for a Dendro-GR checkout; the generated solver directories are `FCCZ4_GR` and `BSSN_GR`, one per formulation, the real host library is Dendrolib, and `dendro5` is the host CMake target whose presence the generated solver refuses to build against; see [Dendro](infrastructures/dendro/index.md). |
