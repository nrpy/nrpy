...
| Dendro | Generated-application infrastructure for standalone Dendrolib applications. Dendrolib represents spatial AMR with balanced octants and decomposes them into padded `ot::Block` regions for stencil evaluation. NRPy emits complete applications `NRPy_BSSN_GR` and `NRPy_fCCZ4_GR`; see [Dendro](infrastructures/dendro/index.md) and [Octree Grid, AMR, And Time Stepping](infrastructures/dendro/grid-amr-and-time-stepping.md). |
