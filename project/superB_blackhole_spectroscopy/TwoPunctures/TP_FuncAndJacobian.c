// FuncAndJacobian.c from TwoPunctures. Note that this C file is just a collection of functions for computing Jacobians. TP_FuncAndJacobian() is
// unused.
...
static inline REAL min(REAL const x, REAL const y) { return x < y ? x : y; }
...
void Derivatives_AB3(int nvar, int n1, int n2, int n3, derivs v) {
  ...
  for (ivar = 0; ivar < nvar; ivar++) {
    ...
    for (i = 0; i < n1; i++) { /* Calculation of Derivatives w.r.t. phi-Dir. (Fourier)*/
      for (j = 0; j < n2; j++) {
        for (k = 0; k < n3; k++) {
...
void F_of_v(ID_persist_struct par, int nvar, int n1, int n2, int n3, derivs v, REAL *F, derivs u) {
  ...
  int i, j, k, ivar, indx;
  REAL al, be, A, B, X, R, x, r, phi, y, z, Am1, *values;
  derivs U;
  REAL *sources;
  ...
  /* ZACH DISABLED:
     ...
     REAL *s_x, *s_y, *s_z;
     int i3D;
     s_x    =calloc(n1*n2*n3, sizeof(REAL));
     s_y    =calloc(n1*n2*n3, sizeof(REAL));
     s_z    =calloc(n1*n2*n3, sizeof(REAL));
     for (i = 0; i < n1; i++)
     for (j = 0; j < n2; j++)
     for (k = 0; k < n3; k++)
     {
     ...
     C_To_c (par, nvar, X, R, &(s_x[i3D]), &r, U);
  ...
  */
  ...
  REAL psi, psi2, r_plus, r_minus; // psi4, psi7
  FILE *debugfile = NULL;
  if (par.do_residuum_debug_output) // ZACH REMOVED: && CCTK_MyProc(cctkGH) == 0)
  {
  ...
  for (i = 0; i < n1; i++) {
    for (j = 0; j < n2; j++) {
      for (k = 0; k < n3; k++) {
        ...
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        C_To_c(par, nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        rx3_To_xyz(nvar, x, r, phi, &y, &z, U);
        NonLinEquations(par, sources[Index(0, i, j, k, 1, n1, n2, n3)], A, B, X, R, x, r, phi, y, z, U, values);
        for (ivar = 0; ivar < nvar; ivar++) {
          indx = Index(ivar, i, j, k, nvar, n1, n2, n3);
        ...
        if (debugfile && (k == 0)) {
          ...
          // psi4 = psi2 * psi2;
          // psi7 = psi * psi2 * psi4;
          fprintf(debugfile, "%.16g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n", (double)x, (double)y, (double)A, (double)B,
                  (double)(U.d11[0] + U.d22[0] + U.d33[0] +
                           /*                      0.125 * BY_KKofxyz (x, y, z) / psi7 +*/
                           (2.0 * Pi / psi2 / psi * sources[indx]) * FAC),
                  (double)((U.d11[0] + U.d22[0] + U.d33[0]) * FAC), (double)(-(2.0 * Pi / psi2 / psi * sources[indx]) * FAC), (double)sources[indx]
                  /*(double)F[indx]*/
          );
        }
      }
...
/* --------------------------------------------------------------------------*/
void J_times_dv(ID_persist_struct par, int nvar, int n1, int n2, int n3, derivs dv, REAL *Jdv,
                derivs u) { /*      Calculates the left hand sides of the non-linear equations F_m(v_n)=0*/
  /*      and the function u (u.d0[]) as well as its derivatives*/
  /*      (u.d1[], u.d2[], u.d3[], u.d11[], u.d12[], u.d13[], u.d22[], u.d23[], u.d33[])*/
  /*      at interior points and at the boundaries "+/-"*/
  int i, j, k, ivar, indx;
...
/* --------------------------------------------------------------------------*/
void JFD_times_dv(ID_persist_struct par, int i, int j, int k, int nvar, int n1, int n2, int n3, derivs dv, derivs u,
                  REAL *values) { /* Calculates rows of the vector 'J(FD)*dv'.*/
  ...
  int ivar, indx;
  REAL al, be, A, B, X, R, x, r, phi, y, z, Am1;
  REAL sin_al, sin_al_i1, sin_al_i2, sin_al_i3, cos_al;
  REAL sin_be, sin_be_i1, sin_be_i2, sin_be_i3, cos_be;
  REAL dV0, dV1, dV2, dV3, dV11, dV12, dV13, dV22, dV23, dV33, ha, ga, ga2, hb, gb, gb2, hp, gp, gp2, gagb, gagp, gbgp;
  derivs dU, U;
  ...
  Am1 = A - 1;
  for (ivar = 0; ivar < nvar; ivar++) {
    int iccc = Index(ivar, i, j, k, nvar, n1, n2, n3), ipcc = Index(ivar, i + 1, j, k, nvar, n1, n2, n3),
        imcc = Index(ivar, i - 1, j, k, nvar, n1, n2, n3), icpc = Index(ivar, i, j + 1, k, nvar, n1, n2, n3),
        icmc = Index(ivar, i, j - 1, k, nvar, n1, n2, n3), iccp = Index(ivar, i, j, k + 1, nvar, n1, n2, n3),
        iccm = Index(ivar, i, j, k - 1, nvar, n1, n2, n3), icpp = Index(ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
        icmp = Index(ivar, i, j - 1, k + 1, nvar, n1, n2, n3), icpm = Index(ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
        icmm = Index(ivar, i, j - 1, k - 1, nvar, n1, n2, n3), ipcp = Index(ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
        imcp = Index(ivar, i - 1, j, k + 1, nvar, n1, n2, n3), ipcm = Index(ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
        imcm = Index(ivar, i - 1, j, k - 1, nvar, n1, n2, n3), ippc = Index(ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
        impc = Index(ivar, i - 1, j + 1, k, nvar, n1, n2, n3), ipmc = Index(ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
          ...
          for (i1 = i_0; i1 <= i_1; i1++) {
            for (j1 = j_0; j1 <= j_1; j1++) {
              for (k1 = k_0; k1 <= k_1; k1++) {
                JFD_times_dv(par, i1, j1, k1, nvar, n1, n2, n3, dv, u, values);
...
void calculate_derivs(int i, int j, int k, int ivar, int nvar, int n1, int n2, int n3, derivs v, derivs vv) {
  REAL al = Pih * (2 * i + 1) / n1, be = Pih * (2 * j + 1) / n2, sin_al = sin(al), sin2_al = sin_al * sin_al, cos_al = cos(al), sin_be = sin(be),
       sin2_be = sin_be * sin_be, cos_be = cos(be);
...
REAL interpol(REAL a, REAL b, REAL c, derivs v) {
  return v.d0[0] + a * v.d1[0] + b * v.d2[0] + c * v.d3[0] + 0.5 * a * a * v.d11[0] + a * b * v.d12[0] + a * c * v.d13[0] + 0.5 * b * b * v.d22[0] +
         b * c * v.d23[0] + 0.5 * c * c * v.d33[0];
}

/* --------------------------------------------------------------------------*/
static REAL clamp_pm_one(REAL val) { return val < -1 ? -1 : val > 1 ? 1 : val; }
...
REAL PunctTaylorExpandAtArbitPosition(ID_persist_struct par, int ivar, int nvar, int n1, int n2, int n3, derivs v, REAL x, REAL y, REAL z) {
  REAL xs, ys, zs, rs2, phi, X, R, A, B, al, be, aux1, aux2, a, b, c, result, Ui;
  int i, j, k;
  derivs vv;
...
REAL PunctIntPolAtArbitPosition(ID_persist_struct par, int ivar, int nvar, int n1, int n2, int n3, derivs v, REAL x, REAL y, REAL z) {
  REAL xs, ys, zs, rs2, phi, X, R, A, B, aux1, aux2, result, Ui;
...
void SpecCoef(int n1, int n2, int n3, int ivar, REAL *v, REAL *cf) {
  // VASILIS: Here v is a pointer to the values of the variable v at the collocation points and cf_v a pointer to the spectral coefficients that this
  // routine calculates

  int i, j, k, N, n, l;
  REAL *p, ***values3, ***values4;
