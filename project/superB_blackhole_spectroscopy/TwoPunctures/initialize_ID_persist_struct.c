...
void initialize_ID_persist_struct(commondata_struct *restrict commondata, ID_persist_struct *restrict par) {
  // Step 1: Set default TwoPunctures values
  ...
  snprintf(par->grid_setup_method, 100,
           "evaluation"); // How to fill the 3D grid from the spectral grid. evaluation is the only accurate one; don't use the other option!
